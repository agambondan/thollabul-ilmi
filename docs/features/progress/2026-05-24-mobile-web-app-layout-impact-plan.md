...
## Web App Sholat Tracker Route Dashboard Parity

Status: completed for the native `web_app` Sholat Tracker route visual and
interaction parity pass.

Implemented:

- `ExploreScreen` keeps the classic Sholat Tracker card/list flow unchanged for
  the paper layout.
- In `web_app` mode, Sholat Tracker now renders through
  `WebAppSholatTrackerRoute`, a dedicated dashboard tracker surface modeled
  after `/dashboard/sholat-tracker`: route header, today progress card, five
  prayer toggle rows, 7-day summary, and monthly heatmap.
- Removed Sholat Tracker from the generic local tool-route config so the native
  `web_app` route cannot regress to the old generic shell plus classic content.
- Added route coverage to ensure the tracker surface renders independently from
  `renderFeatureContent` and still calls the shared `togglePrayer` handler.

Verification:

```bash
node --check apps/mobile/src/screens/explore/WebAppSholatTrackerRoute.js
node --check apps/mobile/src/screens/explore/ExploreWebAppRoutes.js
node --check apps/mobile/src/screens/ExploreScreen.js
cd apps/mobile
npm test -- exploreWebAppRoutes.test.js --runInBand
...
- `node --check` passed for the touched route/screen files.
- Targeted route test passed: 1 suite, 10 tests.
- Full mobile Jest passed: 48 suites, 698 tests.
- Feature parity checker passed: 50 manifest features, 14 utility routes, 43
  mobile feature keys, 154 web app routes scanned, 64 dashboard page routes
  scanned.
- `git diff --check` passed.
...
## Web App Hadith Theme Parity

Status: completed for the native `web_app` Hadith light/dark palette pass.

Implemented:

- `HadithScreen` now reads `isDarkTheme` in addition to `isWebAppLayout`.
- The `web_app` Hadith route keeps the existing dark dashboard palette when
  dark mode is active, and switches the list background, search field, tab
  pills, book shelf cards, summary/results surfaces, empty states, load-more
  button, and detail hero to a light dashboard palette when the app theme is
  light.
- Added regression coverage for both light and dark `web_app` Hadith palettes.
- Classic Hadith list/detail routing, fetching, filters, and bookmark behavior
  remain unchanged.
...
cd ../..
node scripts/check-feature-parity.js
git diff --check -- apps/mobile/src/screens/HadithScreen.js apps/mobile/src/__tests__/hadithScreen.test.js docs/WEB_MOBILE_SYNC.md docs/features/progress/2026-05-24-mobile-web-app-layout-impact-plan.md
find apps/mobile/src -name '*.js' -print0 | xargs -0 wc -l | sort -nr | head -10
```

Results:

- `node --check` passed for `HadithScreen.js`.
- Targeted Hadith/shell tests passed: 2 suites, 38 tests.
- Full mobile Jest passed: 48 suites, 713 tests.
- Feature parity checker passed: 50 manifest features, 14 utility routes, 43
  mobile feature keys, 154 web app routes scanned.
...
- `QiblaScreen` now reads `isDarkTheme` in addition to `isWebAppLayout`.
- The `web_app` Qibla route keeps the existing light dashboard palette when
  light mode is active, and switches the route surface, header, manual-location
  panel, compass panel, metric tiles, location card, status messages, inputs,
  and helper note to a dark dashboard palette when the app theme is dark.
- Added regression coverage for both light and dark `web_app` Qibla palettes.
- Classic Qibla GPS permission, manual coordinate, compass animation/math, and
  refresh behavior remain unchanged.

Verification:

```bash
node --check apps/mobile/src/screens/QiblaScreen.js
cd apps/mobile
npm test -- QiblaScreen.test.js mobileAppShell.test.js --runInBand
npm test -- --runInBand
cd ../..
node scripts/check-feature-parity.js
git diff --check -- apps/mobile/src/screens/QiblaScreen.js apps/mobile/src/__tests__/QiblaScreen.test.js docs/WEB_MOBILE_SYNC.md docs/features/progress/2026-05-24-mobile-web-app-layout-impact-plan.md
find apps/mobile/src -name '*.js' -print0 | xargs -0 wc -l | sort -nr | head -10
```

Results:

- `node --check` passed for `QiblaScreen.js`.
- Targeted Qibla/shell tests passed: 2 suites, 35 tests.
- Full mobile Jest passed: 48 suites, 723 tests.
- Feature parity checker passed: 50 manifest features, 14 utility routes, 43
  mobile feature keys, 154 web app routes scanned.
- `git diff --check` passed for touched files.
- Line-count gate stayed clean: no `apps/mobile/src/**/*.js` file is above
...
## Web App Khatam Theme Parity

Status: completed for the native `web_app` Khatam light/dark palette pass.

Implemented:

- `KhatamScreen` now reads `isDarkTheme` in addition to `isWebAppLayout`.
- The `web_app` Khatam route keeps the existing light dashboard palette when
  light mode is active, and switches the route surface, guest/empty cards,
  tracker header, target card, target stats, juz grid card, and status message
  to a dark dashboard palette when the app theme is dark.
- Added regression coverage for both light and dark `web_app` Khatam palettes.
- Classic Khatam progress loading, target persistence, and continue-reading
  behavior remain unchanged.

Verification:

```bash
node --check apps/mobile/src/screens/KhatamScreen.js
cd apps/mobile
npm test -- khatam.test.js mobileAppShell.test.js --runInBand
npm test -- --runInBand
cd ../..
node scripts/check-feature-parity.js
git diff --check -- apps/mobile/src/screens/KhatamScreen.js apps/mobile/src/__tests__/khatam.test.js docs/WEB_MOBILE_SYNC.md docs/features/progress/2026-05-24-mobile-web-app-layout-impact-plan.md
find apps/mobile/src -name '*.js' -print0 | xargs -0 wc -l | sort -nr | head -10
```

Results:

- `node --check` passed for `KhatamScreen.js`.
- Targeted Khatam/shell tests passed: 2 suites, 28 tests.
- Full mobile Jest passed: 48 suites, 719 tests.
- Feature parity checker passed: 50 manifest features, 14 utility routes, 43
  mobile feature keys, 154 web app routes scanned.
- `git diff --check` passed for touched files.
- Line-count gate stayed clean: no `apps/mobile/src/**/*.js` file is above
