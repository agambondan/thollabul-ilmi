...
| Riwayat login multi-device | Tracked | Current-device session sudah tampil. Riwayat login butuh endpoint/backend table audit session, jadi tidak bisa ditutup hanya dari mobile. |
| Delete account self-service | Tracked | Masih kontak tim untuk verifikasi data personal. Perlu product/security decision sebelum self-service destructive action. |
