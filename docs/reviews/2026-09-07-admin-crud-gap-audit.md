...
- [ ] Tambah CRUD API `panduan-sholat`.
- [ ] Tambah halaman `/admin/panduan-sholat`.
- [ ] Tambah link Panduan Sholat di grup Worship admin nav.
- [ ] Pastikan field `arabic`, `transliteration`, `translation_text`, `description`, `source`, `notes`, `step` bisa diedit.
- [ ] Tambah validasi step unik dan reorder sederhana.
- [ ] Tambah audit log saat panduan sholat diubah.

### Gelombang 2 — Master data ringan

- [ ] Tambah CRUD API/admin untuk `achievements`.
- [ ] Tambah CRUD API/admin untuk master `amalan` item.
- [ ] Tambah filter kategori/status aktif untuk amalan.
