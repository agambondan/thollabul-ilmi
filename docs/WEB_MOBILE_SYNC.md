...
| Perpustakaan Ilmu | `/library`, `/library/:slug` | `/dashboard/library`, `/dashboard/library/:slug` | Web and mobile both expose the same public catalog through `GET /api/v1/library/books` with paginated loading, while admin uses `GET /api/v1/library/admin/books` so draft resources remain manageable without leaking to public catalog. Dashboard/detail journeys keep notes, bookmarks, source/license verification metadata, study progress badges/filters, and resume surfaces inside the personal shell with `library_book` references and `/api/v1/library/progress`; mobile mirrors progress filters in the feature list. |
