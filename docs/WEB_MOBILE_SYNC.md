...
| Mobile web-inspired layout | `web_app` is now implemented as an opt-in native mobile shell plus dashboard-aligned surfaces for Home and Quran list, plus first-pass surfaces for Quran reader/detail, Hadith, Ibadah, Prayer, Qibla, Khatam, Explore/Belajar, Global Search, and Profile. Native mobile `classic` layout remains supported and must keep feature parity while visual polish continues incrementally. |
