...
| Mobile web-inspired layout | `web_app` is now the default native mobile shell for new installs, with dashboard-aligned surfaces for Home, Quran list, Hadith book shelf, and Global Search/Cari, plus first-pass surfaces for Quran reader/detail, Hadith detail, Ibadah, Prayer, Qibla, Khatam, Explore/Belajar, and Profile. Native mobile `classic` layout remains supported from settings and must keep feature parity while visual polish continues incrementally. |
