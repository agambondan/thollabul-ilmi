...
| Admin analytics | `/admin` | admin-only | Web now records page-view events through `POST /api/v1/analytics/page-view` and renders admin visitor metrics from `GET /api/v1/analytics/admin/summary`. The admin dashboard also derives review queue, content health, and content status charts from existing admin list endpoints. Mobile does not mirror admin analytics because this is an admin web surface, not a public/mobile feature. |
