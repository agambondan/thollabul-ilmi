...
func Handle(app *fiber.App, repo *repository.Repositories) {
	...
	newMasjidController := controllers.NewMasjidController(newServices)
	newRadioIslamicController := controllers.NewRadioIslamicController(newServices)
	...
	newKajianBookmarkController := controllers.NewKajianBookmarkController(newServices)
	newKajianNoteController := controllers.NewKajianNoteController(newServices)
	...
	master.Get("/kajian/categories", newKajianController.GetCategories)
	...
	// Masjid (public read, admin write)
	master.Get("/masjids", newMasjidController.FindAll)
	master.Get("/masjids/nearby", newMasjidController.FindNearby)
	master.Get("/masjids/:id", newMasjidController.FindByID)
	master.Post("/masjids", admin, newMasjidController.Create)
	master.Put("/masjids/:id", middlewares.EditorOrAdminMiddleware(), newMasjidController.Update)
	master.Delete("/masjids/:id", admin, newMasjidController.Delete)

	// Radio Islamic (public read, admin write)
	master.Get("/radio-islamic", newRadioIslamicController.FindAll)
	master.Get("/radio-islamic/:id", newRadioIslamicController.FindByID)
	master.Post("/radio-islamic", admin, newRadioIslamicController.Create)
	master.Put("/radio-islamic/:id", middlewares.EditorOrAdminMiddleware(), newRadioIslamicController.Update)
