...
func seedFiqhItemsRelated(db *gorm.DB) {
	...
	for _, cat := range data {
		...
		for i := range cat.items {
			...
			if cat.items[i].Dalil == "" {
				cat.items[i].Dalil = cat.items[i].Source
			}
