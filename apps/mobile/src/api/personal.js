...
export const getLibraryProgressList = async () => {
    const payload = await requestJson("/api/v1/library/progress", {
        auth: true,
    });
    return pickItems(payload);
};

export const saveLibraryProgress = async ({
    bookId,
    currentPage = 0,
    note = "",
    status = "reading",
...
export const getNotesByType = async (refType) => {
    const payload = await requestJson(`/api/v1/notes?ref_type=${refType}`, {
        auth: true,
    });
    return pickItems(payload);
};
...
export const deleteNote = async (id) =>
    deleteJson(`/api/v1/notes/${id}`, { auth: true });

export const getNotificationSettings = async () => {
    const payload = await requestJson("/api/v1/notifications/settings", {
        auth: true,
    });
    return pickItems(payload);
};
...
export const getNotificationInbox = async () => {
    const payload = await requestJson("/api/v1/notifications/inbox", {
        auth: true,
    });
    return {
...
export const getMyAchievements = async () => {
    const payload = await requestJson("/api/v1/achievements/mine", {
        auth: true,
    });
    return pickItems(payload);
};
...
export const getMyPoints = () =>
    requestJson("/api/v1/achievements/points", { auth: true });

export const getMyStreak = () => requestJson("/api/v1/streak", { auth: true });

export const logActivity = (type) =>
    postJson("/api/v1/activity", { type }, { auth: true });

export const checkAmalan = (id) =>
    putJson(`/api/v1/amalan/${id}/check`, {}, { auth: true });

export const getTilawahSummary = () =>
    requestJson("/api/v1/tilawah/summary", { auth: true });
...
export const getHafalanSummary = async () =>
...
export const saveMurojaahResult = async ({
    surahId,
    fromAyah,
    toAyah,
    score,
...
export const createUserWird = async (payload) =>
    postJson("/api/v1/user-wird", payload, { auth: true });

export const updateUserWird = async (id, payload) =>
    putJson(`/api/v1/user-wird/${id}`, payload, { auth: true });

export const deleteUserWird = async (id) =>
    deleteJson(`/api/v1/user-wird/${id}`, { auth: true });
...
export const deleteFaraidh = async (id) =>
    deleteJson(`/api/v1/faraidh/simpan/${id}`, { auth: true });
...
export const getKalkulasiZakat = async () => {
    const payload = await requestJson("/api/v1/zakat/kalkulasi", {
        auth: true,
    });
    return pickItems(payload);
};

export const deleteKalkulasiZakat = async (id) =>
    deleteJson(`/api/v1/zakat/kalkulasi/${id}`, { auth: true });
