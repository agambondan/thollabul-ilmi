...
export const requestJson = async (path, options = {}) => {
    ...
    try {
        ...
        throw new Error(
            `Network request failed ke ${url}. Pastikan HP bisa membuka ${API_URL}/api/v1/surah?size=1`,
        );
    }
...
const normalizeAudio = (item, index = 0) => ({
    ...
    qari_slug:
        item?.qari_slug ?? item?.qariSlug ?? item?.slug ?? `qari-${index}`,
...
export const getAyahsForSurahPage = async (
    ...
    const payload = await requestJson(
        `/api/v1/ayah/surah/number/${surahNumber}?size=${size}&page=${page}`,
    );
    const items = dedupeAyahs(
...
const groupMufrodatByAyah = (items) => {
    ...
    map.forEach((group) =>
        group.words.sort((a, b) => (a.wordIndex ?? 0) - (b.wordIndex ?? 0)),
    );
...
export const searchGlobal = async (
    ...
    return {
        ...
        hadithTotal: Number(
            data?.hadith_total ?? (type === "hadith" ? total : 0),
        ),
        dictionaries: pickItems(data?.dictionaries ?? []).map(
            normalizeDictionary,
        ),
        dictionaryTotal: Number(
            data?.dictionary_total ??
                (type === "dictionary" || type === "kamus" ? total : 0),
        ),
        doas: pickItems(data?.doas ?? []).map(normalizeDoa),
        doaTotal: Number(
            data?.doa_total ??
                (type === "doa" || type === "dua" || type === "prayer"
                    ? total
                    : 0),
        ),
        ...
        kajianTotal: Number(
            data?.kajian_total ??
                (type === "kajian" || type === "study" || type === "lesson"
                    ? total
                    : 0),
        ),
        ...
        perawiTotal: Number(
            data?.perawi_total ??
                (type === "perawi" || type === "rawi" || type === "rijal"
                    ? total
                    : 0),
        ),
        total,
...
const searchCollection = async ({
    ...
    try {
        const payload = await requestJson(
            appendQuery(endpoint, queryParam, query),
        );
...
const pagedResult = (payload, page, size) => {
    ...
    const currentPage =
        Number(payload?.page ?? payload?.data?.page ?? page) || 0;
    const total =
        Number(payload?.total ?? payload?.data?.total ?? items.length) ||
        items.length;
    const totalPages = Number(
        payload?.total_pages ??
            payload?.totalPages ??
    ...
    const isLast =
        payload?.last === true ||
        (totalPages ? currentPage + 1 >= totalPages : items.length < size);
...
export const getHadithPage = async ({
    bookSlug = null,
    page = 0,
    size = 20,
    updatedAfter = null,
...
export const normalizeDoa = (item) => ({
    ...
    meta: [item.category, item.occasion, item.source]
        .filter(Boolean)
        .join(" · "),
});
...
export const normalizeKajian = (item) => ({
    ...
    meta: [item.speaker, item.ustadz, item.category, item.date]
        .filter(Boolean)
        .join(" · "),
});
...
export const searchDoa = async (query, { limit = 8 } = {}) =>
    searchCollection({
        ...
        pickSearchText: (item) =>
            [item.title, item.body, item.meta, item.arabic]
                .filter(Boolean)
                .join(" "),
        query,
    });
...
export const searchKajian = async (query, { limit = 8 } = {}) =>
    searchCollection({
        ...
        pickSearchText: (item) =>
            [item.title, item.body, item.meta].filter(Boolean).join(" "),
        query,
    });
...
export const getRelatedHadiths = async (hadith) => {
    const candidates = [
        hadith?.themeId
            ? `/api/v1/hadiths/theme/${hadith.themeId}?size=8`
            : null,
        hadith?.chapterId
            ? `/api/v1/hadiths/chapter/${hadith.chapterId}?size=8`
            : null,
        hadith?.bookSlug
            ? `/api/v1/hadiths/book/${hadith.bookSlug}?size=8`
            : null,
    ].filter(Boolean);
