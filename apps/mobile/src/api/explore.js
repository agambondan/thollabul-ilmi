...
const pickPaginationMeta = (payload, pagination, itemCount = 0) => {
    ...
    if (meta) {
        return {
            hasMore: Boolean(
                meta.has_more ?? meta.hasMore ?? meta.has_next ?? meta.hasNext,
            ),
...
export const normalizeExploreItem = (item, index = 0) => {
    ...
    if (
        ...
        const perawiName = pickText(
            item?.perawi?.nama_latin,
            item?.perawi?.nama_arab,
            item?.perawi?.nama_lengkap,
        );
        const penilaiName = pickText(
            item?.penilai?.nama_latin,
            item?.penilai?.nama_arab,
            item?.penilai?.nama_lengkap,
        );
    ...
    if (item?.prayers || item?.timings) {
        ...
        return {
            ...
            title: pickText(
                item?.date,
                item?.day ? `Tanggal ${item.day}` : "",
                `Jadwal ${index + 1}`,
            ),
            arabic: "",
            body: joinMeta(
                prayers.imsak ?? prayers.Imsak,
                prayers.fajr ?? prayers.Fajr,
                prayers.maghrib ?? prayers.Maghrib,
            ),
            meta: pickText(
                item?.hijri,
                item?.city,
...
export const searchDictionary = async (query) => {
    ...
    const payload = await requestJson(
        `/api/v1/dictionary?q=${encodeURIComponent(query.trim())}&size=20`,
    );
    return pickItems(payload).map(normalizeExploreItem);
};
