...
export const readStoredMobileLanguage = async (
    fallback = defaultMobileLanguage,
) =>
    normalizeMobileLanguage(
        await readPreference(
            preferenceKeys.appLanguage,
            normalizeMobileLanguage(fallback),
        ),
    );
...
export function MobileLocaleProvider({ children }) {
    ...
    const refreshLanguage = useCallback(async () => {
        ...
        const nextLanguage = normalizeMobileLanguage(
            user?.preferred_lang ?? storedLanguage,
        );
