...
const isGenericHadithLabel = (value) =>
    /^hadis(?:\s+\d+)?$/i.test(cleanHadithText(value));
...
const formatHadithGrade = (grade) => {
    ...
    if (normalized === "hasan sahih" || normalized === "hasan shahih")
        return "Hasan Shahih";
...
const getHadithBookLabel = (hadith) => {
    const rawBook = cleanHadithText(
        hadith?.book || hadith?.bookName || hadith?.collection || "",
    );
...
const getHadithTopicLabel = (hadith) => {
    const topic = cleanHadithText(
        hadith?.chapterName || hadith?.themeName || hadith?.title || "",
    );
...
export function HadithScreen({ deepLinkTarget, isActive, navigation }) {
    ...
    const [perawiPanel, setPerawiPanel] = useState({
        guru: [],
        jarhTadil: [],
        loading: false,
        murid: [],
    });
    const [expandedPerawiList, setExpandedPerawiList] = useState({
        guru: false,
        murid: false,
    });
    ...
    const [hadithActionSheet, setHadithActionSheet] = useState({
        visible: false,
        hadith: null,
    });
        ...
        const items = (await getOfflineItems("hadith"))
            ...
            .sort((a, b) => {
                if (a.bookSlug !== b.bookSlug)
                    return String(a.bookSlug).localeCompare(String(b.bookSlug));
                return (
                    Number(a.number ?? a.id ?? 0) -
                    Number(b.number ?? b.id ?? 0)
                );
            });
    ...
    const load = useCallback(
        async ({
            ...
            try {
                ...
                const result = await getHadithPage({
                    bookSlug,
                    page,
    ...
    const loadBooks = useCallback(async () => {
        try {
            ...
            const overview = await getOfflineOverview();
            setBooks(
                Array.isArray(overview.hadithBooks) ? overview.hadithBooks : [],
            );
        }
    }, []);
    ...
    const loadBookmarks = useCallback(async () => {
        ...
        try {
            ...
            setBookmarks(mapped);
            setBookmarkItems(
                items.filter((item) => item.ref_type === "hadith"),
            );
        } catch {
            setBookmarks({});
            setBookmarkItems([]);
    ...
    const selectedBookName =
        books.find((book) => book.slug === selectedBook)?.name ?? "Semua kitab";
    ...
    const loadMoreHadiths = useCallback(() => {
        if (loading || loadingMore || loadingMoreRef.current || detailLoading)
            return;
    ...
    const renderCompactHadithCard = (hadith, meta = "") => {
        ...
        const hasTopic =
            topicLabel &&
            normalizeSearchText(topicLabel) !== normalizeSearchText(bookLabel);
        const bodyText = cleanHadithText(
            hadith.translation || hadith.arabic || "Teks hadis belum tersedia.",
        );
        ...
        return (
            <ContentCard
                ...
                footer={
                    noteCounts[hadith.id] ? (
                        <Text style={styles.compactMeta}>
                            {noteCounts[hadith.id]} catatan
                        </Text>
                    ) : null
                }
            />
                    ...
                    <Pressable
                        ...
                        style={[
                            styles.webAppHadithTab,
                            {
                                backgroundColor:
                                    activeTab === tab.key
                                        ? webAppTheme.activeTab
                                        : webAppTheme.tab,
                            },
                            activeTab === tab.key
                                ? styles.webAppHadithTabActive
                                : null,
                        ]}
                    ...
                    >
                        <Text
                            style={[
                                ...
                                activeTab === tab.key
                                    ? styles.webAppHadithTabTextActive
                                    : null,
                            ]}
                        >
                            {tab.label}
        ...
        <View style={styles.webAppHadithHeader}>
            ...
            <View
                style={[
                    styles.webAppHadithSearch,
                    {
                        ...
                        borderColor: webAppTheme.searchBorder,
                    },
                ]}
            ...
            >
                ...
                <TextInput
                    ...
                    style={[
                        styles.webAppHadithSearchInput,
                        { color: webAppTheme.input },
                    ]}
                    value={query}
                />
            </View>
    ...
    const renderWebAppBookCard = (book) => {
        ...
        const shortLabel = getHadithBookShortLabel({
            book: book.name,
            bookSlug: book.slug,
        });
        ...
        return (
            ...
            >
                ...
                <View style={styles.webAppBookCopy}>
                    ...
                    >
                        <Text style={styles.webAppBookActionText}>
                            Buka Reader
                        </Text>
                    </Pressable>
                </View>
    ...
    const renderWebAppHadithResults = () => (
        <>
            ...
            >
                ...
                <Text numberOfLines={1} style={styles.webAppQueryBadge}>
                    {summaryBadge}
                </Text>
            </View>
                ...
                <>
                    ...
                    >
                        {visibleHadiths.map((hadith) => (
                            <View
                                key={`${hadith.id}-${hadith.title}`}
                                style={styles.hadithListItem}
                            >
                                {renderCompactHadithCard(
                                    hadith,
                                    "web-app-list",
                                )}
                            </View>
    ...
    const sanadPerawi = sanad
        ...
        .filter(
            (perawi, index, all) =>
                perawi?.id &&
                all.findIndex((item) => item?.id === perawi.id) === index,
        );
            ...
            {HADITH_DETAIL_TABS.map((tab) => (
                <Pressable
                    ...
                    style={[
                        ...
                        detailTab === tab.key
                            ? styles.detailTabButtonActive
                            : null,
                        isWebAppLayout && detailTab === tab.key
                            ? styles.webAppDetailTabButtonActive
                            : null,
                        isWebAppLayout && detailTab === tab.key
    ...
    useEffect(() => {
        ...
        const fromList = hadiths.find(
            (item) => String(item.id) === String(hadithId),
        );
    ...
    if (selectedHadith) {
        ...
        const selectedHadithTitle =
            getHadithTopicLabel(selectedHadith) ||
            `${selectedHadithBook} ${selectedHadithNumber}`;
        const detailScreenTitle = isWebAppLayout
            ? "Detail Hadith"
            : "Detail Hadis";
        ...
        return (
            ...
            >
                <View
                    testID={
                        isWebAppLayout
                            ? "hadith-web-app-detail"
                            : "hadith-classic-detail"
                    }
                />
                {isWebAppLayout ? (
                    <View
                    ...
                    >
                        ...
                        <View style={styles.webAppDetailMetaRow}>
                            <Text
                                style={[
                                    styles.webAppDetailMetaChip,
                                    {
                                        backgroundColor: webAppTheme.metaBg,
                                        borderColor: webAppTheme.border,
                                        color: webAppTheme.text,
                                    },
                                ]}
                ...
                {detailTab === "text" ? (
                    <>
                        <Card>
                            <CardTitle meta={selectedHadithGrade}>
                                {selectedHadithTitle}
                            </CardTitle>
                        ...
                        <Card>
                            <CardTitle meta={`${relatedHadiths.length} item`}>
                                Hadis Terkait
                            </CardTitle>
                            {detailLoading && relatedHadiths.length === 0 ? (
                                ...
                                relatedHadiths.map((item) =>
                                    renderCompactHadithCard(item, "related"),
                                )
                            )}
                        </Card>
                    </>
                ...
                {detailTab === "sanad" ? (
                    <Card>
                        <CardTitle meta={`${sanad.length} jalur`}>
                            Sanad
                        </CardTitle>
                ...
                {detailTab === "narrators" ? (
                    <Card>
                        <CardTitle meta={`${sanadPerawi.length} perawi`}>
                            Perawi
                        </CardTitle>
                        {sanadPerawi.length ? (
                        ...
                        {selectedPerawi ? (
                            <View style={styles.narratorPanel}>
                                <SectionHeader
                                    title={
                                        selectedPerawi.nama_latin ||
                                        selectedPerawi.nama_lengkap ||
                                        `Perawi ${selectedPerawi.id}`
                                    }
                                    meta={
                                        selectedPerawi.tabaqah ||
                                        selectedPerawi.status ||
                                        "perawi"
                                    }
                                />
                                ...
                                <Text style={styles.chainMeta}>
                                    {[
                                        ...
                                        selectedPerawi.tahun_wafat
                                            ? `w. ${selectedPerawi.tahun_wafat} H`
                                            : null,
                                    ]
                                        .filter(Boolean)
                                        .join(" · ") || "Profil perawi"}
                ...
                {detailTab === "takhrij" ? (
                    <Card>
                        <CardTitle meta={`${takhrij.length} rujukan`}>
                            Takhrij
                        </CardTitle>
                ...
                {detailTab === "ayat" ? (
                    <Card>
                        <CardTitle meta={`${hadithAyahs.length} ayat`}>
                            Ayat Terkait
                        </CardTitle>
                        {detailLoading && hadithAyahs.length === 0 ? (
                            ...
                            hadithAyahs.map((item) => (
                                ...
                                >
                                    ...
                                    <Text style={styles.referenceMeta}>
                                        Ketuk untuk membuka ayat di Al-Qur'an.
                                    </Text>
                                </Pressable>
                            ))
                        )}
                ...
                {detailTab === "notes" ? (
                    <Card>
                        <CardTitle meta={user ? "Pribadi" : "Masuk akun"}>
                            Catatan
                        </CardTitle>
                        {user ? (
                            <NotesPanel
    ...
    return (
        ...
        >
            ...
            {user && savedHadiths.length > 0 ? (
                <Card>
                    <CardTitle meta={`${savedHadiths.length} tersimpan`}>
                        Hadis Tersimpan
                    </CardTitle>
                    {savedHadiths.map((item) =>
                        renderCompactHadithCard(item, "saved"),
                    )}
                </Card>
            ) : null}
