...
const DEFAULT_BADGES = [
    {
        code: "tilawah_first",
        description: "Mulai perjalanan tilawah.",
        icon: "📖",
        label: "Tilawah Perdana",
        unlocked: false,
    },
    {
        code: "sholat_full",
        description: "Sempurnakan catatan sholat harian.",
        icon: "✅",
        label: "Sholat Penuh",
        unlocked: false,
    },
    {
        code: "starter",
        description: "Akun belajar sudah aktif.",
        icon: "🌟",
        label: "Penuntut Ilmi",
        unlocked: true,
    },
    {
        code: "streak_7",
        description: "Jaga aktivitas belajar selama 7 hari.",
        icon: "🔥",
        label: "Streak 7 Hari",
        unlocked: false,
    },
...
const THEME_OPTIONS = [
    {
        ...
        metaKey: "theme.system.meta",
    },
    {
        key: "light",
        labelKey: "theme.light.label",
        metaKey: "theme.light.meta",
    },
...
const LANGUAGE_OPTIONS = [
    {
        key: "idn",
        labelKey: "language.indonesia.label",
        metaKey: "language.indonesia.meta",
    },
    {
        key: "en",
        labelKey: "language.english.label",
        metaKey: "language.english.meta",
    },
];
...
const LAYOUT_OPTIONS = [
    {
        key: "classic",
        labelKey: "layout.classic.label",
        metaKey: "layout.classic.meta",
    },
    {
        key: "web_app",
        labelKey: "layout.webApp.label",
        metaKey: "layout.webApp.meta",
    },
];
...
const formatSessionDate = (value, fallback, language) => {
    ...
    return date.toLocaleDateString(
        mobileDateLocales[language] ?? mobileDateLocales.idn,
        {
            day: "2-digit",
            month: "short",
            year: "numeric",
...
const normalizeAchievement = (item = {}, options = {}) => {
    ...
    const code =
        source.code ??
        `${source.id ?? item.achievement_id ?? source.name ?? "achievement"}`;
    return {
        code,
...
const getAchievementProgress = (achievement, stats, t) => {
    ...
    if (achievement.category === "streak") {
        ...
        return {
            current,
            label: t("profile.achievements.progress.days", {
    ...
    if (achievement.category === "hafalan") {
        ...
        return {
            current,
            label: t("profile.achievements.progress.surah", {
                current: Math.min(current, threshold),
                threshold,
            }),
            pct: Math.min(100, Math.round((current / threshold) * 100)),
        ...
        >
            ...
            >
                <View
                    testID={
                        isWebAppLayout
                            ? "profile-web-app-subscreen"
                            : "profile-classic-subscreen"
                    }
                />
                {children}
            </ScrollView>
...
function AppearanceSettings({ onUserUpdated, user }) {
    ...
    const saveLanguage = async (nextLanguage) => {
        ...
        try {
            ...
            if (user) {
                const updatedUser = getResponseUser(
                    await updateProfile({ preferredLang: storedLanguage }),
                );
    ...
    return (
        <>
            ...
            {message ? (
                <Text style={styles.settingsStatus}>{message}</Text>
            ) : null}
        </>
    );
}
...
function SecuritySettings({ onDeleteAccount, onSignOut, user }) {
    ...
    if (!user) {
        return (
            <Card>
                <Text style={styles.appearanceLabel}>
                    {t("profile.security.guest.title")}
                </Text>
                <Text style={styles.appearanceMeta}>
        ...
        <>
                ...
                <View style={styles.sessionDeviceCard}>
                    ...
                    <View style={styles.sessionDeviceBody}>
                        ...
                        <Text style={styles.sessionDeviceMeta}>
                            {user.email ||
                                t("profile.security.session.currentMeta")}
                        </Text>
                    </View>
                </View>
                {sessionsLoading ? (
                            ...
                            <View key={item.id} style={styles.sessionListRow}>
                                <View style={styles.sessionListInfo}>
                                    <Text style={styles.sessionListTitle}>
                                        {item.current
                                            ? t(
                                                  "profile.security.session.currentDevice",
                                              )
                                            : t(
                                                  "profile.security.session.loginSession",
                                              )}
                                    </Text>
                                    <Text style={styles.sessionListMeta}>
                                        {t(
                                            "profile.security.session.activeSince",
                                            {
                                                date: formatSessionDate(
                                                    item.created_at,
                                                    t(
                                                        "profile.security.session.now",
                                                    ),
                                                    language,
                                                ),
                                            },
                                ...
                                {item.current ? (
                                    <Text style={styles.sessionCurrentPill}>
                                        {t("profile.security.session.active")}
                                    </Text>
                                ) : (
                                    <Pressable
                                    ...
                                    >
                                        <Text style={styles.sessionRevokeText}>
                                            {revokingSessionId === item.id
                                                ? "..."
                                                : t(
                                                      "profile.security.session.revoke",
                                                  )}
                                        </Text>
                                    </Pressable>
                                )}
            ...
            <Card>
                ...
                <View style={styles.formBlock}>
                    <TextInput
                        accessibilityLabel={t(
                            "profile.security.password.current",
                        )}
                    ...
                    <TextInput
                        accessibilityLabel={t(
                            "profile.security.password.confirm",
                        )}
                    ...
                    <Pressable
                        ...
                        style={[
                            styles.formButton,
                            saving && styles.formButtonDisabled,
                        ]}
                    >
                        {saving ? (
            ...
            <Card>
                ...
                >
                    {deleting ? (
                        ...
                        <Text style={styles.formButtonDangerText}>
                            {deleteConfirm
                                ? t("profile.security.delete.confirm")
                                : t("profile.security.delete.action")}
                        </Text>
                    )}
                </Pressable>
            </Card>
        ...
        return (
            ...
            >
                ...
                <View style={styles.webAppAchievementsHeader}>
                    ...
                    <Text style={styles.webAppAchievementsTitle}>
                ...
                {user ? (
                    <View style={styles.webAppAchievementsHero}>
                        <Text style={styles.webAppAchievementsHeroLabel}>
                    ...
                    <View style={styles.webAppAchievementsNotice}>
                        <Text style={styles.webAppAchievementsNoticeIcon}>
                            🏅
                        </Text>
                ...
                {message ? (
                    <Text style={styles.webAppAchievementsHint}>{message}</Text>
                ) : null}
                            ...
                            <View
                                ...
                                style={[
                                    ...
                                    !achievement.unlocked &&
                                        styles.webAppAchievementsCardLocked,
                                ]}
                            >
                                <View
                                    style={[
                                        ...
                                        achievement.unlocked &&
                                            styles.webAppAchievementsBadgeEarned,
                                    ]}
                                >
                                    <Text
                                        style={
                                            styles.webAppAchievementsBadgeIcon
                                        }
                                    >
                                        {achievement.icon || "🏅"}
                                    </Text>
                                ...
                                <View style={styles.webAppAchievementsCardBody}>
                                    <Text
                                        style={
                                            styles.webAppAchievementsCardTitle
                                        }
                                    >
                                        {achievement.label}
                                    </Text>
                                    {achievement.description ? (
                                        <Text
                                            style={
                                                styles.webAppAchievementsCardDescription
                                            }
                                        >
                                            {achievement.description}
                                        </Text>
                                ...
                                <View
                                    style={[
                                        ...
                                        achievement.unlocked &&
                                            styles.webAppAchievementsStatusEarned,
                                    ]}
                                >
                                    <Text
                                        style={[
                                            ...
                                            achievement.unlocked &&
                                                styles.webAppAchievementsStatusTextEarned,
                                        ]}
                                    >
                                        {achievement.unlocked
        ...
        <SubScreen title={t("profile.achievements.title")} onBack={onBack}>
            <Card style={styles.achievementHero}>
                ...
                <View style={styles.achievementHeroBody}>
                    ...
                    <Text style={styles.achievementHeroLabel}>
                        {t("profile.stats.totalPoints")}
                    </Text>
                    ...
                    return (
                        <Card
                            ...
                            style={[
                                ...
                                !achievement.unlocked &&
                                    styles.achievementDetailLocked,
                            ]}
                        >
                            <View style={styles.achievementDetailTop}>
                                <View
                                    style={[
                                        ...
                                        achievement.unlocked &&
                                            styles.achievementDetailIconUnlocked,
                                    ]}
                                >
                                ...
                                <View style={styles.achievementDetailBody}>
                                    <View
                                        style={styles.achievementDetailTitleRow}
                                    >
                                        <Text
                                            style={
                                        ...
                                        <View
                                            style={[
                                                ...
                                                achievement.unlocked &&
                                                    styles.achievementStatePillUnlocked,
                                            ]}
                                        >
                                            <Text
                                                style={[
                                                    ...
                                                    achievement.unlocked &&
                                                        styles.achievementStateTextUnlocked,
                                                ]}
                                            >
                                                {achievement.unlocked
                                                    ? t(
                            ...
                            {progress ? (
                                <View style={styles.achievementProgressBlock}>
                                    <View
                                        style={styles.achievementProgressHeader}
                                    >
                                        <Text
                                            style={
                                        ...
                                        >
                                            {t(
                                                "profile.achievements.progress.label",
                                            )}
                                        </Text>
                                        <Text
                                            style={
                                    ...
                                    <View
                                        style={styles.achievementProgressTrack}
                                    >
                                        <View
                                            style={[
                            ...
                            <View style={styles.achievementRewardRow}>
                                ...
                                <Text style={styles.achievementRewardText}>
                                    {t("profile.achievements.reward", {
                                        points: achievement.rewardPoints,
                                    })}
                                </Text>
                                {achievement.earnedAt ? (
                                    <Text style={styles.achievementEarnedDate}>
                                        {t(
                                            "profile.achievements.alreadyEarned",
                                        )}
                                    </Text>
                                ) : null}
                            </View>
...
export function ProfileScreen({ isActive, navigation, onOpenTab }) {
        ...
        const loadProfileData = async () => {
                ...
                setStats({
                    points:
                        pointsRes.status === "fulfilled"
                            ? (pointsRes.value?.total_points ??
                              pointsRes.value?.points ??
                              pointsRes.value?.data?.total_points ??
                              0)
                            : 0,
                    streak:
                        streakRes.status === "fulfilled"
                            ? (streakRes.value?.current_streak ??
                              streakRes.value?.streak ??
                              streakRes.value?.data?.current_streak ??
                              0)
                            : 0,
                    hafalanCount:
                    ...
                    tilawahPages:
                        tilawahRes.status === "fulfilled"
                            ? (tilawahRes.value?.total_pages ??
                              tilawahRes.value?.pages ??
                              tilawahRes.value?.data?.total_pages ??
                              null)
                            : null,
                });
            } else {
            ...
            const allAchievements =
                allAchievementsRes.status === "fulfilled"
                    ? allAchievementsRes.value.map((item) =>
                          normalizeAchievement(item),
                      )
                    : DEFAULT_BADGES;
            const earnedAchievements =
                myAchievementsRes.status === "fulfilled"
                    ? myAchievementsRes.value.map((item) =>
                          normalizeAchievement(item, { earned: true }),
                      )
                    : [];
    ...
    if (currentScreen === "settings-notifications") {
        return (
            <SubScreen
                title={t("profile.settings.notifications.label")}
                onBack={pop}
            >
                <NotificationCenter />
            </SubScreen>
    ...
    if (currentScreen === "settings-appearance") {
        return (
            <SubScreen
                title={t("profile.settings.appearance.label")}
                onBack={pop}
            >
                <AppearanceSettings
    ...
    if (currentScreen === "settings-security") {
        return (
            <SubScreen
                title={t("profile.settings.security.label")}
                onBack={pop}
            >
                <SecuritySettings
    ...
    if (isWebAppLayout) {
            ...
            ? [
                  {
                      ...
                      onPress: () =>
                          onOpenTab("belajar", { featureKey: "leaderboard" }),
                  },
                  {
                      Icon: Target,
                      key: "goals",
                      ...
                      onPress: () =>
                          onOpenTab("belajar", { featureKey: "goals" }),
                  },
                  {
                      Icon: Settings,
                      key: "settings",
                      label: t("profile.settings.title"),
                      meta: t("profile.actions.settings.meta"),
                  ...
                  },
                  {
                      ...
                      meta: t("profile.actions.logout.meta"),
                      onPress: signOut,
                  },
              ]
            : [
                  ...
                  },
                  {
                      ...
                      key: "settings",
                      label: t("profile.settings.title"),
                      meta: t("profile.actions.localSettings.meta"),
                      onPress: () => push("settings"),
                  },
              ];
        ...
        return (
            <ScrollView
                contentContainerStyle={[
                    styles.webAppProfileContent,
                    { backgroundColor: webAppProfileTheme.bg },
                ]}
                showsVerticalScrollIndicator={false}
                style={[
                    styles.webAppProfileRoot,
                    { backgroundColor: webAppProfileTheme.bg },
                ]}
            ...
            >
                ...
                <View
                    style={[
                        ...
                        {
                            backgroundColor: webAppProfileTheme.surface,
                            borderColor: webAppProfileTheme.border,
                        },
                    ]}
                    testID='profile-web-app-hero'
                >
                    <View style={styles.webAppAvatar}>
                        <Text style={styles.webAppAvatarText}>
                            {initials || "TI"}
                        </Text>
                    </View>
                    <Text
                        style={[
                            styles.webAppEyebrow,
                            { color: webAppProfileTheme.accent },
                        ]}
                    >
                        {t("profile.eyebrow")}
                    </Text>
                    ...
                    <Text
                        style={[
                            styles.webAppProfileEmail,
                            { color: webAppProfileTheme.muted },
                        ]}
                    >
                    ...
                    >
                        ...
                        <Text style={styles.webAppSettingsText}>
                ...
                {stats ? (
                    <View style={styles.webAppStatsGrid}>
                        <View
                            style={[
                                styles.webAppStatTile,
                                {
                                    backgroundColor: webAppProfileTheme.tile,
                                    borderColor: webAppProfileTheme.border,
                                },
                            ]}
                        >
                            ...
                            <Text
                                style={[
                                    styles.webAppStatLabel,
                                    { color: webAppProfileTheme.text },
                                ]}
                            >
                                {t("profile.stats.totalPoints")}
                            </Text>
                            ...
                            {stats.hafalanCount !== null ? (
                                <Pressable
                                    ...
                                    style={[
                                        ...
                                        {
                                            backgroundColor:
                                                webAppProfileTheme.tile,
                                            borderColor:
                                                webAppProfileTheme.border,
                                        },
                                    ]}
                                >
                                    <BookOpen
                    ...
                    <View style={styles.webAppActionGrid}>
                        {accountActions.map((item) => {
                            ...
                            const actionTileStyle = item.danger
                                ? {
                                      backgroundColor:
                                          webAppProfileTheme.actionDangerBg,
                                      borderColor:
                                          webAppProfileTheme.actionDangerBorder,
                                  }
                                : {
                                      backgroundColor: webAppProfileTheme.tile,
                                      borderColor: webAppProfileTheme.border,
                            ...
                            return (
                                <Pressable
                                    ...
                                    style={[
                                        styles.webAppActionTile,
                                        item.danger &&
                                            styles.webAppActionTileDanger,
                                        actionTileStyle,
                                    ]}
                                >
                                    <View
                                        style={[
                                            styles.webAppActionIcon,
                                            item.danger &&
                                                styles.webAppActionIconDanger,
                                            {
                                                backgroundColor: item.danger
                                                    ? "rgba(248, 113, 113, 0.12)"
                                                    : webAppProfileTheme.iconBg,
                                            },
                                        ]}
                                    >
                                        <Icon
                                            color={
                                                item.danger
                                                    ? webAppProfileTheme.actionDangerIcon
                                                    : webAppProfileTheme.accent
                                            }
                                            size={18}
                                            strokeWidth={2.2}
                                        />
                                    ...
                                    <Text
                                        style={[
                                            styles.webAppActionLabel,
                                            item.danger &&
                                                styles.webAppActionLabelDanger,
                                            {
                                                color: item.danger
                                                    ? webAppProfileTheme.actionDangerLabel
                                                    : webAppProfileTheme.title,
                                            },
                                        ]}
                                    >
                                        {item.label}
    ...
    return (
        ...
        >
            <View
                testID={
                    isWebAppLayout
                        ? "profile-web-app-surface"
                        : "profile-classic-surface"
                }
            />
                ...
                <View style={styles.badgeGrid}>
                    {achievements.slice(0, 6).map((badge) => (
                        ...
                        >
                            <View
                                style={[
                                    styles.badgeIconShell,
                                    badge.unlocked &&
                                        styles.badgeIconShellUnlocked,
                                ]}
                            >
