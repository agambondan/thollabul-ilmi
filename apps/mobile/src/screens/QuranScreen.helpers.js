...
export const DISPLAY_MODES = [
    {
        ...
        description:
            "Arab dan terjemah per baris, tanpa info surat dan nomor ayat terpisah.",
    },
    {
        key: "card",
        label: "Grid",
        title: "Grid ayat",
        description:
            "Arab dan terjemah dalam kartu ringkas dengan menu sejajar ayat.",
    },
    {
        key: "focus",
        label: "Fokus",
...
export const ARABIC_FONTS = [
    { key: "kitab", label: "Uthmani", fontFamily: QURAN_FONT_FAMILIES.kitab },
    {
        key: "indopak",
        label: "Indopak",
        fontFamily: QURAN_FONT_FAMILIES.indopak,
    },
...
export const stripHtmlTags = (value = "") =>
    decodeArabicHtml(value.replace(/<[^>]+>/g, ""));
...
export const parseTajweedHtml = (html = "") => {
    if (!html || !/<tajweed/i.test(html))
        return [{ text: stripHtmlTags(html), className: null }];

    const segments = [];
    const tagPattern =
        /<tajweed\s+class=["']([^"']+)["']\s*>(.*?)<\/tajweed>/gis;
    let lastIndex = 0;
    let match;
    ...
    while ((match = tagPattern.exec(html)) !== null) {
        if (match.index > lastIndex) {
            segments.push({
                text: stripHtmlTags(html.slice(lastIndex, match.index)),
                className: null,
            });
        }
    ...
    if (lastIndex < html.length) {
        segments.push({
            text: stripHtmlTags(html.slice(lastIndex)),
            className: null,
        });
    }
...
export const getAyahIdentity = (ayah) =>
    `${ayah.surahNumber ?? "surah"}:${ayah.number ?? ayah.id}`;
...
export const getFirstPageNumber = (items, fallback = MUSHAF_FIRST_PAGE) => {
    const pageNumber = items.find((ayah) =>
        Number.isFinite(Number(ayah.pageNumber)),
    )?.pageNumber;
...
export const getInitialSurahPages = (targetPage, totalAyahs) => {
    ...
    const maxPage =
        Number(totalAyahs) > 0
            ? Math.max(
                  0,
                  Math.floor((Number(totalAyahs) - 1) / SURAH_PAGE_SIZE),
              )
            : page + SURAH_TARGET_PREFETCH_RADIUS;
    const start = Math.max(0, page - SURAH_TARGET_PREFETCH_RADIUS);
    const end = Math.min(
        maxPage,
        Math.max(start, page + SURAH_TARGET_PREFETCH_RADIUS),
    );
    return Array.from({ length: end - start + 1 }, (_, index) => start + index);
};

export const getMushafTranslationLength = (ayah) =>
    String(ayah.translation || "").length;
...
export const getMushafFragmentLength = (fragment) =>
    fragment.segments.reduce(
        (total, token) => total + getMushafTokenLength(token),
        0,
    );
...
export const splitMushafAyahFragments = (ayah, maxArabicLength) => {
    ...
    return fragments.length
        ? fragments
        : [{ ayah, fragmentIndex: 0, segments: [], isAyahEnd: true }];
};
    ...
    items.forEach((ayah) => {
        splitMushafAyahFragments(ayah, maxArabicLength).forEach((fragment) => {
            const fragmentLength = Math.max(
                1,
                getMushafFragmentLength(fragment),
            );
            const translationLength = fragment.isAyahEnd
                ? getMushafTranslationLength(ayah)
                : 0;
...
export const getCompactArabicSurahName = (value) =>
    `${value ?? ""}`.replace(SURAH_PREFIX_PATTERN, "");
...
export const TAJWEED_GROUPS = [
    {
        ...
        title: "Idgham",
        description:
            "Memasukkan nun sukun/tanwin ke huruf berikutnya hingga menjadi satu",
        rules: [
            {
        ...
        rules: [
            {
                ...
                title: "Haqiqi",
                description:
                    "15 huruf ikhfa (selain ب dan huruf idgham/idzhar)",
        ...
        rules: [
            {
                ...
                description:
                    "Mad asli — huruf mad (ا و ي) tanpa hamzah atau sukun sesudahnya",
                example: "قَالَ",
            },
            {
                key: "mad-wajib",
            ...
            },
            {
                ...
                description:
                    "Huruf mad diikuti sukun lazim (asli) atau tasydid",
        ...
        rules: [
            {
                ...
                title: "Sughra",
                description:
                    "Huruf qalqalah sukun di tengah kata (pantulan ringan)",
                example: "يَقْتُلُونَ",
            },
            {
                key: "qalqalah-kubra",
                ...
                title: "Kubra",
                description:
                    "Huruf qalqalah di akhir kata saat waqf (pantulan kuat)",
                example: "أَحَدٌ",
            },
        ],
    },
