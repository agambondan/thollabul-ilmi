    ...
    >
      <View style={styles.cardHeader}>
        <View style={styles.badges}>
          {type ? <Text style={[styles.badge, type === 'video' && styles.badgeVideo]}>{type}</Text> : null}
...
function KajianStat({ accent = '#047857', formatValue, label, value }) {
  return (
    <View style={styles.statCard}>
      ...
      <Text style={[styles.statValue, { color: accent }]}>{formatValue(value)}</Text>
    </View>
  );
}
    ...
    >
      ...
      <View style={styles.categories}>
        ...
        {categories.map((category) => (
          <Pressable
            key={category}
            onPress={() => onSelectCategory(kajianCategory === category ? '' : category)}
            style={[styles.category, kajianCategory === category && styles.categoryActive]}
            testID={`web-app-kajian-category-${category}`}
          >
            <Text style={[styles.categoryText, kajianCategory === category && styles.categoryTextActive]}>
              {category}
            </Text>
          </Pressable>
        ))}
