...
function Field({ field, label, multiline = false, placeholder, setUserWirdForm, value }) {
  return (
    <View style={styles.field}>
      <Text style={styles.fieldLabel}>{label}</Text>
      <TextInput
        multiline={multiline}
        onChangeText={(nextValue) => setUserWirdForm((current) => ({ ...current, [field]: nextValue }))}
    ...
    <View style={styles.wirdCard} testID="web-app-user-wird-card">
      <View style={styles.wirdHeader}>
        ...
        <View style={styles.wirdTitleBlock}>
          <Text numberOfLines={1} style={styles.wirdTitle}>{item.title}</Text>
      ...
      {transliteration ? <Text style={styles.wirdTransliteration}>{transliteration}</Text> : null}
      {translation ? <Text style={styles.wirdTranslation}>{translation}</Text> : null}
...
export function WebAppUserWirdRoute({
  ...
  return (
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} style={styles.root}>
      ...
      <Modal animationType="fade" onRequestClose={closeForm} transparent visible={formVisible}>
