...
const getAsbabunAyahNumber = (item) =>
    item?.raw?.ayah_number ??
    item?.raw?.ayah_start ??
    item?.raw?.ayah_refs?.[0]?.ayah_number ??
    item?.raw?.ayah_id;
...
const getAsbabunAyahLabel = (item, t) => {
    ...
    const end =
        item?.raw?.ayah_end ??
        item?.raw?.ayah_refs?.[item?.raw?.ayah_refs?.length - 1]?.ayah_number;
...
export function WebAppTafsirRoute({
    ...
    const selectedSurah = surahs.find(
        (surah) => Number(surah.number) === selectedNumber,
    );
    const typedSurahNumber = Number.parseInt(surahSearch, 10);
    const canSubmitAsbabun =
        Number.isInteger(typedSurahNumber) &&
        typedSurahNumber >= 1 &&
    ...
    const searchTestID = isAsbabun
        ? "web-app-asbabun-search"
        : "web-app-tafsir-search";
    const surahCardTestID = isAsbabun
        ? "web-app-asbabun-surah-card"
        : "web-app-tafsir-surah-card";
    ...
    const loadingText = isAsbabun
        ? t("explore.tafsir.asbabunLoading")
        : t("explore.tafsir.loading");
    const placeholder = isAsbabun
        ? t("explore.tafsir.asbabunPlaceholder")
        : t("explore.tafsir.searchPlaceholder");
    const emptyResultText = isAsbabun
        ? t("explore.tafsir.asbabunEmpty")
        : t("explore.tafsir.empty");
    const showInitialAsbabunState =
        isAsbabun && !selectedSurahNumber && !loading && !error;
        ...
        >
            ...
            <View style={[styles.header, isAsbabun && styles.headerCentered]}>
                ...
                <View
                    style={[
                        styles.headerText,
                        isAsbabun && styles.headerTextCentered,
                    ]}
                >
                    <Text
                        style={[
            ...
            <View style={isAsbabun ? styles.asbabunSearchRow : null}>
                ...
                {isAsbabun ? (
                    <Pressable
                        ...
                        style={[
                            styles.asbabunSubmit,
                            (!canSubmitAsbabun || loading) &&
                                styles.asbabunSubmitDisabled,
                        ]}
                        testID='web-app-asbabun-submit'
                    >
                        <Text style={styles.asbabunSubmitText}>
                            {loading ? "..." : t("explore.tafsir.searchAction")}
                        </Text>
                    </Pressable>
                ) : null}
            </View>
            ...
            {isAsbabun ? (
                <View style={styles.quickSection}>
                    <Text style={styles.quickLabel}>
                        {t("explore.tafsir.quickExamples")}
                    </Text>
