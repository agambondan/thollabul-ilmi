...
function PrayerRow({ done, item, onPress }) {
  return (
    ...
    >
      ...
      <Text style={[styles.prayerLabel, done && styles.prayerLabelDone]}>{item.label}</Text>
    </Pressable>
  );
}
      ...
      <View style={styles.monthCard}>
        ...
        <View style={styles.monthGrid}>
          {monthDays.map((entry) => (
            <View
              key={entry.day}
              style={[styles.monthDay, getHeatStyle(entry), entry.today && styles.monthDayToday]}
              testID="web-app-sholat-month-day"
            >
              <Text style={[styles.monthDayText, getHeatTextStyle(entry)]}>{entry.day}</Text>
            </View>
          ))}
        </View>
