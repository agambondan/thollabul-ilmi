...
const cleanTime = (value) => (value ? String(value).replace(/ \(.*\)$/, '') : '-');
...
function ImsakiyahRow({ index, item, t }) {
  ...
  return (
    <View style={[styles.row, isToday && styles.rowToday]} testID="web-app-imsakiyah-row">
      <View style={styles.dayCell}>
        <Text style={[styles.dayText, isToday && styles.dayTextToday]}>{getDayLabel(row, index)}</Text>
        {isToday ? <View style={styles.todayDot} /> : null}
      </View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.timesScroller}>
...
export function WebAppImsakiyahRoute({
  error,
  items = [],
  loading,
}) {
