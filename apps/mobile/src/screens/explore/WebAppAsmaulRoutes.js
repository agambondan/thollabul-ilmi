import { ChevronLeft, ChevronRight, Hand, RefreshCcw, RotateCcw, Shuffle } from 'lucide-react-native';
...
const getArabic = (name) => name?.arabic ?? name?.translation?.ar ?? name?.name ?? '';
const getLatin = (name) => name?.latin ?? name?.transliteration ?? name?.translation?.latin_idn ?? '';
const getMeaning = (name) => name?.indonesian ?? name?.meaning ?? name?.translation?.idn ?? name?.translation?.en ?? '';
const sumCounts = (counts = {}) => Object.values(counts).reduce((sum, count) => sum + (Number(count) || 0), 0);

const clampIndex = (index, length) => Math.max(0, Math.min(Math.max(0, length - 1), index));
...
function IconButton({ disabled, Icon, label, onPress, variant = 'neutral' }) {
  return (
    <Pressable
      ...
      disabled={disabled}
      onPress={onPress}
      style={[styles.iconButton, variant === 'primary' && styles.iconButtonPrimary, disabled && styles.disabledButton]}
    >
      <Icon color={variant === 'primary' ? '#ffffff' : '#475569'} size={17} strokeWidth={2.3} />
    </Pressable>
  );
}
...
export function WebAppAsmaulFlashcardRoute({
  ...
  const sequence = order.length === names.length ? order : names.map((_, index) => index);
  ...
  const move = (delta) => {
    ...
    setAsmaulIndex((current) => clampIndex(current + delta, sequence.length || names.length));
  };

  const shuffle = () => {
    const next = names.map((_, index) => index).sort(() => Math.random() - 0.5);
    setOrder(next);
    setShuffled(true);
    setAsmaulFlashcardRevealed(false);
    setAsmaulIndex(0);
      ...
      {asmaulLoading || !currentName ? (
        ...
        <>
          <View style={styles.toolbar}>
            <Text style={styles.toolbarText}>{safeIndex + 1} / {sequence.length}</Text>
          ...
          >
            ...
            {asmaulFlashcardRevealed ? (
              <>
                <Text style={styles.flashLatin}>{getLatin(currentName)}</Text>
...
export function WebAppAsmaulWiridRoute({
  ...
  const updateCount = (nextCount) => {
    if (!nameId) return;
    if (vibrate) {
      if (nextCount === WIRID_TARGET || nextCount % 33 === 0) hapticMedium();
      else hapticTap();
    }
      ...
      {asmaulLoading || !currentName ? (
        ...
        <>
          <View style={styles.counterCard}>
            ...
            <Text style={styles.wiridArabic}>{getArabic(currentName)}</Text>
            <Text style={styles.wiridLatin}>{getLatin(currentName)}</Text>
            <Text style={styles.wiridMeaning}>{getMeaning(currentName)}</Text>
            ...
            <View style={styles.counterWrap}>
              ...
              >
                ...
                <Text style={styles.counterTarget}>/ {WIRID_TARGET}</Text>
              </Pressable>
            </View>
            ...
            <View style={styles.progressBlock}>
              ...
              <Text style={styles.progressText}>{progressPct}% · {count}/{WIRID_TARGET}</Text>
            </View>
            ...
            <View style={styles.actionRow}>
              ...
              <Pressable
                onPress={() => {
                  setAsmaulCounts({});
                  setAsmaulWiridCount({}, nameId, 0).catch(e => console.error(e));
                }}
