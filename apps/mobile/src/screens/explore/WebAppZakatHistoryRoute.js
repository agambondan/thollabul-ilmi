...
const formatDate = (item, language) => {
  ...
  if (!value) return '';
  try {
    return new Date(value).toLocaleDateString(language === 'en' ? 'en-US' : 'id-ID', { day: 'numeric', month: 'short', year: 'numeric' });
  } catch {
    return '';
  }
};
    ...
    <View key={item.id} style={[styles.faraidhHistoryCard, { marginTop: spacing.xs }]}>
      <View style={{ flex: 1 }}>
        ...
        <Text style={{ fontSize: 11, color: item.is_local ? colors.muted : colors.primary }}>
          {item.is_local ? t('explore.zakatHistory.localDevice') : t('explore.zakatHistory.syncedAccount')}
        </Text>
        <Text style={{ fontSize: 11, color: colors.muted }}>{formatDate(item, language)}</Text>
      </View>
      <Pressable onPress={() => onDelete(item)} style={[styles.heirButton, { borderColor: colors.danger }]}>
        <Text style={[styles.heirButtonText, { color: colors.danger }]}>{t('explore.zakatHistory.delete')}</Text>
      </Pressable>
    </View>
  );
}

function WebAppZakatHistoryItem({ formatCurrency, item, language, onDelete, t }) {
  ...
  return (
    <View key={item.id} style={styles.webAppZakatHistoryCard} testID="web-app-zakat-history-card">
      <View style={styles.webAppZakatHistoryCardHeader}>
        <View style={styles.webAppZakatHistoryTypeRow}>
          ...
          <View style={styles.webAppZakatHistoryTypeText}>
            <Text style={styles.webAppZakatHistoryType}>{item.nama_jenis || t('explore.zakatHistory.typeFallback')}</Text>
            <Text style={styles.webAppZakatHistoryDate}>{formatDate(item, language) || t('explore.zakatHistory.dateUnavailable')}</Text>
          </View>
        </View>
        <Pressable
          accessibilityLabel={t('explore.zakatHistory.deleteAccessibility', { title: item.nama_jenis || t('explore.zakatHistory.itemFallback') })}
          hitSlop={10}
      ...
      <View style={styles.webAppZakatHistoryMeta}>
        {wealth > 0 ? <Text style={styles.webAppZakatHistoryMetaText}>{t('explore.zakatHistory.totalWealth', { amount: formatCurrency(wealth) })}</Text> : null}
        {itemNisab > 0 ? <Text style={styles.webAppZakatHistoryMetaText}>{t('explore.zakatHistory.nisab', { amount: formatCurrency(itemNisab) })}</Text> : null}
      ...
      <View style={styles.webAppZakatHistoryAmountRow}>
        ...
        <Text style={[styles.webAppZakatHistoryStatus, isPaid ? styles.webAppZakatHistoryStatusPaid : styles.webAppZakatHistoryStatusUnpaid]}>
          {isPaid ? t('explore.zakatHistory.paid') : t('explore.zakatHistory.unpaid')}
        </Text>
      </View>
    </View>
  );
  ...
  return (
    <View style={styles.webAppZakatHistorySurface} testID="explore-web-app-zakat-history-surface">
      <Pressable onPress={onBack} style={styles.webAppZakatHistoryBack} testID="web-app-zakat-history-back">
        ...
        <Text style={styles.webAppZakatHistoryBackText}>{t('explore.zakatHistory.back')}</Text>
      </Pressable>
      ...
      <View style={styles.webAppZakatHistoryHero}>
        ...
        <Text style={styles.webAppZakatHistoryTitle}>{t('explore.zakatHistory.title')}</Text>
        <Text style={styles.webAppZakatHistorySubtitle}>{t('explore.zakatHistory.subtitle')}</Text>
      </View>
      ...
      <View style={styles.webAppZakatHistoryStats}>
        <View style={styles.webAppZakatHistoryStatCard}>
          ...
          <Text style={styles.webAppZakatHistoryStatLabel}>{t('explore.zakatHistory.historyCount')}</Text>
        </View>
        <View style={styles.webAppZakatHistoryStatCard}>
          <Text style={styles.webAppZakatHistoryStatValue}>{formatCurrency(historyTotal)}</Text>
          <Text style={styles.webAppZakatHistoryStatLabel}>{t('explore.zakatHistory.totalZakat')}</Text>
        </View>
      </View>
      ...
      {zakatHistory.length === 0 ? (
        <View style={styles.webAppZakatHistoryEmpty}>
          <Text style={styles.webAppZakatHistoryEmptyTitle}>{t('explore.zakatHistory.emptyTitle')}</Text>
          <Text style={styles.webAppZakatHistoryEmptyText}>{t('explore.zakatHistory.emptyText')}</Text>
        </View>
      ) : (
        <View style={styles.webAppZakatHistoryList}>
          {zakatHistory.map((item) => (
            <WebAppZakatHistoryItem formatCurrency={formatCurrency} item={item} key={item.id} language={language} onDelete={onDelete} t={t} />
          ))}
        </View>
      )}
    </View>
