...
const getCategory = (item) =>
  pickText(getRaw(item).category?.name, getRaw(item).category?.title, getRaw(item).category, item?.meta);
...
const getCorrectAnswer = (item) => {
  const raw = getRaw(item);
  return raw.correct_answer ?? raw.answer_key ?? raw.answer ?? raw.correctAnswer ?? raw.correct_answer_index;
};
...
function OptionButton({ disabled, index, item, onPress, option, selected }) {
  ...
  return (
    ...
    >
      ...
      {answered && correct ? <CheckCircle2 color="#047857" size={18} strokeWidth={2.3} /> : null}
      {answered && isSelected && !correct ? <XCircle color="#dc2626" size={18} strokeWidth={2.3} /> : null}
    </Pressable>
  );
}
...
export function WebAppQuizRoute({
  ...
  const selected = currentItem ? answers[getItemId(currentItem, currentIndex)] : null;
  const options = useMemo(() => parseOptions(currentItem), [currentItem]);
  const score = typeof scoreQuiz === 'function'
    ? scoreQuiz()
    : items.reduce((sum, item, index) => sum + (getSelectedCorrect(item, answers[getItemId(item, index)]) ? 1 : 0), 0);
      ...
      {!loading && total && !done ? (
        <>
          ...
          {getCategory(currentItem) ? (
            <Text style={styles.categoryBadge}>{getCategory(currentItem)}</Text>
          ) : null}
          ...
          {selected ? (
            <View style={[
              ...
              getSelectedCorrect(currentItem, selected) ? styles.explanationCorrect : styles.explanationWrong,
            ]}>
              <Text style={[
