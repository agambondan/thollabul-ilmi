import { ArrowLeft, History, Plus, Save, Scale, Trash2 } from 'lucide-react-native';
    ...
    <View style={styles.heirCard}>
      ...
      <View style={styles.heirActions}>
        <Pressable
          ...
          style={[styles.stepperButton, count === 0 && styles.disabledButton]}
        ...
        <Pressable
          ...
          onPress={() => onChange(field.key, 1)}
          style={[styles.stepperButton, count >= field.max && styles.disabledButton]}
...
function Notice({ children, tone = 'amber' }) {
  return (
    <View style={[styles.notice, tone === 'blue' ? styles.noticeBlue : styles.noticeAmber]}>
      <Text style={[styles.noticeText, tone === 'blue' ? styles.noticeTextBlue : styles.noticeTextAmber]}>{children}</Text>
    </View>
  );
}
...
export function WebAppFaraidhRoute({ context }) {
  ...
  const setHeir = (key, delta) => {
    ...
    setFaraidh((current) => {
      ...
      const nextValue = Math.min(field?.max ?? 20, Math.max(0, currentValue + delta));
      return { ...current, heirs: { ...current.heirs, [key]: nextValue } };
    });
  };
      ...
      <View style={styles.card}>
        ...
        <View style={styles.actions}>
          <Pressable
            ...
            onPress={handleSave}
            style={[styles.primaryButton, (savingFaraidh || distributable <= 0) && styles.disabledButton]}
