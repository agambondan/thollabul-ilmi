...
const getFilterCategory = (item) =>
  toStr(getRaw(item).category ?? getRaw(item).jenis_nilai ?? getRaw(item).type ?? getRaw(item).occasion ?? item?.meta)
    .toLowerCase()
    .replace(/\s+/g, '_')
    .trim();
...
function CategoryPill({ active, label, onPress, testID }) {
  return (
    <Pressable onPress={onPress} style={[styles.categoryPill, active && styles.categoryPillActive]} testID={testID}>
...
function ReferenceCard({ config, index, item, onOpen, t }) {
  ...
  return (
    <Pressable onPress={() => onOpen(item)} style={styles.card} testID="web-app-reference-card">
      <View style={styles.cardTop}>
        {leading ? (
          <View style={styles.leadingBadge}>
            <Text numberOfLines={2} style={styles.leadingText}>{leading}</Text>
          </View>
        ) : null}
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        {config.arabicHeading ? <Text style={styles.headerArabic}>{config.arabicHeading}</Text> : null}
