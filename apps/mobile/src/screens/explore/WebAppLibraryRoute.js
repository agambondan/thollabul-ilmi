...
const uniqueValues = (items, resolver) =>
  Array.from(new Set(items.map(resolver).filter(Boolean))).sort((a, b) => a.localeCompare(b));
...
function ProgressSummary({ items, progressMap }) {
  ...
  const trackedBooks = progressItems
    .map((progress) => {
      ...
      const book = items.find((item) => String(getBookId(item)) === String(bookId));
    ...
    <View style={styles.progressPanel}>
      ...
      {trackedBooks.length ? (
        <View style={styles.progressList}>
          {trackedBooks.slice(0, 6).map(({ book, bookId, progress }) => (
            <View key={`${bookId}-${progress?.status ?? 'reading'}`} style={styles.progressCard}>
...
function LibraryCard({ index, item, onOpen, progress }) {
  ...
  return (
    <Pressable onPress={() => onOpen(item)} style={styles.card} testID="web-app-library-card">
      <View style={styles.cardTop}>
        <View style={styles.bookIcon}>
          <BookOpen color={ACCENT} size={19} strokeWidth={2.1} />
        </View>
        {meta.sourceUrl ? <ExternalLink color="#d1d5db" size={16} strokeWidth={2.2} /> : null}
      </View>
      ...
      <View style={styles.metaRow}>
        {meta.author ? <Text numberOfLines={1} style={styles.meta}>{meta.author}</Text> : null}
        {meta.category ? <Text numberOfLines={1} style={styles.meta}>{meta.category}</Text> : null}
        {meta.level ? <Text numberOfLines={1} style={styles.meta}>{meta.level}</Text> : null}
        {meta.format ? <Text numberOfLines={1} style={[styles.meta, styles.metaUpper]}>{meta.format}</Text> : null}
      </View>
    </Pressable>
  );
}
...
export function WebAppLibraryRoute({
  ...
  const categories = useMemo(() => uniqueValues(items, (item) => getBookMeta(item).category), [items]);
  const levels = useMemo(() => uniqueValues(items, (item) => getBookMeta(item).level), [items]);
        ...
        <View style={styles.filterGroup}>
          ...
          {LIBRARY_PROGRESS_STATUSES.map((item) => (
            <FilterPill
              ...
              key={item.key}
              label={item.label}
              onPress={() => onSelectProgressFilter(libraryProgressFilter === item.key ? '' : item.key)}
