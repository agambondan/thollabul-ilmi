...
const formatGregorianHomeDate = (date) => {
    try {
        return new Intl.DateTimeFormat("id-ID", homeDateFormatOptions).format(
            date,
        );
    } catch {
...
const formatHijriHomeDate = (hijri, fallbackDate) => {
    ...
    return parts.length
        ? parts.join(" ")
        : formatFallbackHijriHomeDate(fallbackDate);
};
...
const formatCountdown = (secondsDelta) => {
    ...
    const minutes = `${Math.floor((secondsDelta % 3600) / 60)}`.padStart(
        2,
        "0",
    );
...
const locationErrorLabels = new Set([
    "LOKASI NONAKTIF",
...
const areCoordsClose = (first, second) =>
    Boolean(
        first &&
        second &&
        Math.abs(first.lat - second.lat) < 0.001 &&
        Math.abs(first.lng - second.lng) < 0.001,
    );
...
const normalizePrayerCache = (value, dateKey) => {
    ...
    if (
        value?.method !== homePrayerMethod ||
        value?.madhab !== homePrayerMadhab
    )
        return null;
...
export function HomeScreen({ isActive, navigation, onOpenTab }) {
    ...
    const contextualShortcuts = useMemo(() => {
        ...
        if (hour >= 4 && hour < 12) {
            items.push({
                Icon: Sun,
                featureKey: "dzikir",
                label: t("home.shortcut.dhikrMorning.label"),
                sub: t("home.shortcut.dhikrMorning.sub"),
                tab: "belajar",
            });
            ...
            items.push({
                Icon: Sunset,
                featureKey: "dzikir",
                label: t("home.shortcut.dhikrEvening.label"),
                sub: t("home.shortcut.dhikrEvening.sub"),
                tab: "belajar",
            });
        }
        ...
        if (locationActive) {
            items.push({
                Icon: Compass,
                label: t("home.shortcut.qibla.label"),
                params: { view: "qibla" },
                sub: locationLabel,
                tab: "ibadah",
            });
        ...
        if (recentFeatures.some((f) => f.key === "quran")) {
            items.push({
                Icon: FileText,
                featureKey: "tafsir",
                label: t("home.shortcut.tafsir.label"),
                sub: t("home.shortcut.tafsir.sub"),
                tab: "belajar",
            });
        }
    ...
    const directoryGroups = useMemo(() => {
        const primaryRows = [
            {
                Icon: Search,
                key: "global-search",
                subtitle: t("home.directory.globalSearch.subtitle"),
                title: t("home.directory.globalSearch.title"),
                type: "internal",
                view: "global-search",
            },
            {
                Icon: BookOpen,
                key: "quran",
                subtitle: t("home.directory.quran.subtitle"),
                title: t("home.directory.quran.title"),
                type: "tab",
                tab: "quran",
            },
            {
                Icon: Book,
                key: "hadith",
                subtitle: t("home.directory.hadith.subtitle"),
                title: t("home.directory.hadith.title"),
                type: "tab",
                tab: "hadith",
            },
            {
                Icon: Compass,
                key: "qibla",
                subtitle: t("home.directory.prayer.subtitle"),
                title: t("home.directory.prayer.title"),
                type: "tab",
                tab: "ibadah",
                params: { view: "qibla" },
            },
            {
                Icon: Bell,
                key: "notifications",
                subtitle: t("home.directory.notifications.subtitle"),
                title: t("home.directory.notifications.title"),
                type: "feature",
        ...
        return [
            {
                key: "utama",
                label: t("home.directory.group.main"),
                rows: primaryRows,
            },
            ...groupedFeatures,
        ];
    ...
    const hasPrayerSchedule =
        nextPrayer.time !== "--:--" && nextPrayer.countdown !== "--:--:--";
    const gregorianDate = useMemo(
        () => formatGregorianHomeDate(dateSnapshot),
        [dateSnapshot],
    );
    ...
    const prayerStatusLabel = useMemo(() => {
        ...
        if (prayerSyncState === "retrying") {
            return t("home.status.retrying", {
                attempt: prayerRetryAttempt,
                total: prayerRetryDelays.length,
            });
        }
    ...
    const fetchPrayerTimesWithRetry = useCallback(
        async function fetchPrayerTimesWithRetryForCoords(
            coords,
            dateKey,
            attempt = 0,
        ) {
            ...
            if (mountedRef.current) {
                ...
                if (attempt === 0) {
                    ...
                    setPrayerMessage(
                        `Jadwal sholat belum tersedia. Mencoba ulang ${attempt}/${prayerRetryDelays.length}...`,
                    );
                }
            }

            try {
                const nextTimes = await getPrayerTimes({
                    ...coords,
                    madhab: homePrayerMadhab,
                    method: homePrayerMethod,
                });
                ...
                if (attempt < prayerRetryDelays.length) {
                    ...
                    setPrayerMessage(
                        `Jadwal sholat belum tersedia. Mencoba ulang ${nextAttempt}/${prayerRetryDelays.length}...`,
                    );
                    prayerRetryTimerRef.current = setTimeout(() => {
                        fetchPrayerTimesWithRetryForCoords(
                            coords,
                            dateKey,
                            nextAttempt,
                        );
                ...
                setPrayerMessage(
                    "Jadwal sholat belum tersedia. Tarik untuk memuat ulang.",
                );
                return false;
            }
        },
    ...
    const refreshCurrentLocationInBackground = useCallback(
        function refreshCurrentLocationInBackgroundForCoords(
            baseCoords,
            attempt = 0,
        ) {
            ...
            withTimeout(
                ...
                .catch(() => {
                    ...
                    if (attempt < locationRetryDelays.length) {
                        ...
                        setPrayerSyncState("waiting");
                        setPrayerMessage(
                            `GPS masih mencari lokasi. Mencoba lagi ${nextAttempt}/${locationRetryDelays.length}...`,
                        );
                        locationRetryTimerRef.current = setTimeout(() => {
                            refreshCurrentLocationInBackgroundForCoords(
                                null,
                                nextAttempt,
                            );
                    ...
                    setPrayerSyncState("waiting");
                    setPrayerMessage(
                        "GPS belum terbaca. Aktifkan lokasi presisi atau tarik untuk mencoba ulang.",
                    );
                });
        },
    ...
    const loadHomeData = useCallback(
        async ({ refresh = false } = {}) => {
            ...
            try {
                ...
                if (permission.status === "granted") {
                    const [
                        ...
                        readPreference(
                            preferenceKeys.homeLastLocation,
                            null,
                        ).then(normalizeStoredLocation),
                        readPreference(
                            preferenceKeys.homePrayerTimes,
                            null,
                        ).then((value) =>
                            normalizePrayerCache(value, prayerDateKey),
                        ),
                        Promise.resolve(
                    ...
                    const lastKnownCoords =
                        coordsFromPosition(lastKnownPosition);

                    if (lastKnownCoords) {
                        coords = lastKnownCoords;
                        await applyHomeLocation(
                            coords,
                            areCoordsClose(storedLocation, lastKnownCoords)
                    ...
                    if (
                        cachedPrayerTimes &&
                        (!coords ||
                            areCoordsClose(coords, cachedPrayerTimes.coords))
                    ) {
                        if (!coords) {
                    ...
                    if (!coords && mountedRef.current) {
                        ...
                        setPrayerSyncState("waiting");
                        setPrayerMessage(
                            "GPS masih mencari lokasi. Jadwal akan dimuat otomatis saat lokasi terbaca.",
                        );
                    }
                    ...
                    setPrayerMessage(
                        "Aktifkan lokasi untuk melihat jadwal sholat di tempatmu.",
                    );
                }
            } catch {
                if (mountedRef.current) {
                    ...
                    setPrayerSyncState("waiting");
                    setPrayerMessage(
                        "Lokasi belum terbaca. Coba aktifkan GPS lalu tarik untuk memuat ulang.",
                    );
                }
            }

            const [hadithResult, ayahResult, hijriResult, remindersResult] =
            ...
            if (ayahResult.status === "fulfilled" && ayahResult.value?.id) {
                ...
                setDailyAyah({
                    ...
                    ref: [
                        chosen.surahName,
                        chosen.number ? `Ayah ${chosen.number}` : null,
                    ]
                        .filter(Boolean)
                        .join(" · "),
            ...
            if (hijriResult.status === "fulfilled") {
                setHijriDate(
                    formatHijriHomeDate(hijriResult.value, currentDate),
                );
            } else {
            ...
            if (remindersResult.status === "fulfilled") {
                setDailyReminders(
                    Array.isArray(remindersResult.value)
        ...
        [
            ...
            clearPrayerRetryTimer,
            fetchPrayerTimesWithRetry,
            refreshCurrentLocationInBackground,
        ],
    );
    ...
    useEffect(() => {
        ...
        Promise.all([readPinnedFeatures(), readRecentFeatures()]).then(
            ...
            >
                ...
                {directoryGroups.map((group) => (
                    <View key={group.key} style={styles.directoryGroup}>
                        <Text style={styles.directoryGroupTitle}>
                            {group.label}
                        </Text>
                        {group.rows.map((row) => (
                            <ContentCard
