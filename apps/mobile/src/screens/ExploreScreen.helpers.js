...
export const KAJIAN_CATEGORIES = ['aqidah', 'fiqh', 'akhlak', 'tafsir', 'hadits', 'sirah', 'umum'];
...
export const formatCurrency = (value = 0) => `Rp ${Math.round(Number(value) || 0).toLocaleString('id-ID')}`;
export const pickText = (...values) => values.find((value) => typeof value === 'string' && value.trim()) ?? '';
export const stripHtmlText = (value = '') => `${value}`.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
...
export const formatCompactStat = (value = 0) => Number(value || 0).toLocaleString('id-ID');
...
export const getFilteredKajianItems = (items = [], search = '', category = '') => {
...
export const getFilteredBlogItems = (items = [], search = '', category = '') => {
  const query = normalizeSearchText(search);
  return items.filter((item, index) => {
    const text = normalizeSearchText([
...
export const getExploreItemKey = (item) => String(item?.id ?? item?.raw?.id ?? item?.raw?.slug ?? item?.title ?? item?.body);
