...
const searchFilters = [
    ...
    {
        key: "dictionary",
        labelKey: "search.filter.dictionary",
        remoteType: "dictionary",
    },
...
const remoteSearchFilters = searchFilters.filter(
    (item) => !["all", "feature"].includes(item.key),
);
...
const sanitizeRemoteResult = (filterKey, result = emptyGlobalResult) => {
    ...
    const rawTotal = Number(
        result?.[totalField] ?? result?.total ?? items.length,
    );
...
const mergeResultsByFilter = (resultsByFilter) => ({
    ...
    dictionaryTotal: resultCountForFilter(
        resultsByFilter.dictionary,
        "dictionary",
    ),
...
const cleanMeta = (value, fallback) => {
    ...
    if (
        ["backend", "server", "storage", "device", "perangkat"].includes(
            normalized,
        )
    ) {
        return fallback;
...
export function GlobalSearchScreen({
    ...
    const [remoteResultsByFilter, setRemoteResultsByFilter] = useState(
        createEmptyResultsByFilter,
    );
    ...
    const selectedFilter =
        searchFilters.find((item) => item.key === activeFilter) ??
        searchFilters[0];
    const selectedFilterLabel = t(selectedFilter.labelKey);
    const trimmedQuery = query.trim();
    const hasQuery = trimmedQuery.length >= MIN_QUERY_LENGTH;
    const mergedRemoteResults = useMemo(
        () => mergeResultsByFilter(remoteResultsByFilter),
    ...
    const showDictionaries =
        activeFilter === "all" || activeFilter === "dictionary";
    ...
    const filterCounts = {
        all: remoteSearchFilters.reduce(
            (sum, filter) =>
                sum +
                resultCountForFilter(
                    remoteResultsByFilter[filter.key],
                    filter.key,
                ),
            featureResults.length,
        ),
        dictionary: resultCountForFilter(
            remoteResultsByFilter.dictionary,
            "dictionary",
        ),
    ...
    const displayedFeatureResults =
        activeFilter === "all"
            ? featureResults.slice(0, PREVIEW_SIZE)
            : featureResults;
        ...
        const timer = setTimeout(async () => {
            ...
            for (const filter of remoteSearchFilters) {
                const result = await Promise.resolve(
                    runRemoteFilterSearch(filter, trimmedQuery, PREVIEW_SIZE),
                ).then(
            ...
            searchSettled.forEach(({ filter, result }) => {
                if (result.status === "fulfilled") {
                    nextResultsByFilter[filter.key] = sanitizeRemoteResult(
                        filter.key,
                        result.value,
                    );
                } else {
            ...
            const remoteResultCount = remoteSearchFilters.reduce(
                (sum, filter) =>
                    sum +
                    resultCountForFilter(
                        nextResultsByFilter[filter.key],
                        filter.key,
                    ),
                0,
            );
        ...
        runRemoteFilterSearch(filter, trimmedQuery, PAGE_SIZE)
            ...
            .catch(() => {
                if (!cancelled) {
                    setLoading(false);
                    const currentResult =
                        remoteResultsByFilter[activeFilter] ??
                        emptyGlobalResult;
                    const loaded = currentResult[field]?.length ?? 0;
                    if (loaded === 0) {
                        setMessage(
                            t("search.previewFailure", {
    ...
    const handleLoadMore = async () => {
        ...
        setLoadingMore(true);
        const filter = searchFilters.find((item) => item.key === activeFilter);
        if (!filter || filter.remoteType === "all") {
        ...
        if (result.status === "fulfilled") {
            ...
            setRemoteResultsByFilter((prev) => {
                ...
                const existingKeys = new Set(
                    currentItems.map(
                        (item) =>
                            `${item.id ?? item.number ?? JSON.stringify(item)}`,
                    ),
                );
                const appendedItems = (result.value[field] ?? []).filter(
                    (item) => {
            ...
            setMessage(
                t("search.loadFilterError", { filter: selectedFilterLabel }),
            );
        }
        setLoadingMore(false);
    };
    ...
    const searchChips = recentSearches.length
        ? recentSearches
        : quickSuggestions;
    const chipLabel = recentSearches.length
        ? t("search.chips.recent")
        : t("search.chips.quick");
    const resultSummary = t("search.resultSummary", {
        count: totalResults,
        query: trimmedQuery,
    });
    const emptyState =
        emptyStateByFilter[activeFilter] ?? emptyStateByFilter.all;
    const renderSearchResults = () => (
        <>
            <View
                testID={
                    isWebAppLayout
                        ? "global-search-web-app-surface"
                        : "global-search-classic-surface"
                }
            />
            ...
            {showAyahs ? (
                ...
                >
                    {remoteResults.ayahs.map((item) => {
                        const surahName =
                            item.surahName ||
                            surahNameByNumber[Number(item.surahNumber)] ||
                            "";
                        const title =
                            [
                                surahName ||
                                    (item.surahNumber
                                        ...
                                        : null),
                                item.number ? `Ayah ${item.number}` : null,
                            ]
                                .filter(Boolean)
                                .join(" · ") || "Ayat Quran";
                        const meta =
                            [
                                item.juzNumber ? `Juz ${item.juzNumber}` : null,
                                item.pageNumber
                                    ? `Hal. ${item.pageNumber}`
                                    : null,
                            ]
                ...
                >
                    {remoteResults.hadiths.map((item) => (
                        <ResultRow
                            ...
                            meta={cleanMeta(
                                item.book,
                                t("search.filter.hadith"),
                            )}
                            onPress={() =>
                                onOpenTab("hadith", { hadithId: item.id })
                            }
                            subtitle={item.translation}
                            title={
                                item.title ||
                                `${t("search.filter.hadith")} ${item.id}`
                            }
                        />
                    ))}
                    {activeFilter === "all" &&
                ...
                >
                    {remoteResults.kajians.map((item) => (
                        <ResultRow
                            ...
                            meta={cleanMeta(
                                item.meta,
                                t("search.filter.kajian"),
                            )}
                            onPress={() =>
                                onOpenTab("belajar", { featureKey: "kajian" })
                            }
                            subtitle={item.body}
                            title={item.title}
                        />
                ...
                >
                    {displayedFeatureResults.map((feature) => (
                        <ResultRow
                            ...
                            meta={cleanMeta(
                                feature.group,
                                t("search.filter.feature"),
                            )}
                            onPress={() =>
                                onOpenTab("belajar", {
                                    featureKey: feature.key,
                                    focusSearch: feature.type === "kamus",
                                })
                            }
            ...
            {showDictionaries ? (
                <ResultSection
                    ...
                    title={t("search.filter.dictionary")}
                >
                ...
                >
                    {remoteResults.perawis.map((item) => (
                        <ResultRow
                            ...
                            meta={cleanMeta(
                                item.status,
                                t("search.filter.perawi"),
                            )}
                            onPress={() =>
                                onOpenTab("belajar", { featureKey: "perawi" })
                            }
                            subtitle={item.body}
                            title={item.title}
                        />
            ...
            !loading
                ? (() => {
                      ...
                      const totalField = totalFieldByFilter[activeFilter];
                      const total =
                          remoteResults[totalField] ??
                          remoteResults[field]?.length ??
                          0;
    ...
    if (isWebAppLayout) {
        const webAppSearchTheme = isDarkTheme
            ? WEB_APP_SEARCH_DARK
            : WEB_APP_SEARCH_LIGHT;
        const webAppFilterOrder = [
            "all",
            "quran",
            "hadith",
            "doa",
        ...
        const webAppFilters = webAppFilterOrder
            .map((filterKey) =>
                searchFilters.find((filter) => filter.key === filterKey),
            )
            .filter(Boolean);
        ...
        return (
            ...
            >
                <Text
                    style={[
                        styles.webAppSearchTitle,
                        { color: webAppSearchTheme.title },
                    ]}
                >
                    {t("search.title.webApp")}
                </Text>
                <View style={styles.webAppSearchRow}>
                    <View
                        style={[
                            styles.webAppSearchInputWrap,
                            { backgroundColor: webAppSearchTheme.surface },
                        ]}
                    ...
                    >
                        <TextInput
                            ...
                            style={[
                                styles.webAppSearchInput,
                                { color: webAppSearchTheme.inputText },
                            ]}
                            testID='search-input'
                            value={query}
                        />
                    ...
                    <Pressable
                        ...
                        style={[
                            ...
                            { backgroundColor: webAppSearchTheme.active },
                        ]}
                        testID='global-search-web-app-submit'
                    >
                        <Search color='#ffffff' size={15} strokeWidth={2.2} />
                        <Text style={styles.webAppSearchButtonText}>
                            {t("search.submit")}
                        </Text>
                    </Pressable>
                </View>
