...
jest.mock("../api/client", () => ({
    ...
    getAyahsForSurahPage: jest
        .fn()
        .mockResolvedValue({ items: [], hasMore: false, page: 0 }),
...
jest.mock("../storage/preferences", () => ({
    ...
    readPreference: jest.fn((key, defaultValue) =>
...
beforeEach(() => {
    ...
    preferences.readPreference.mockImplementation((key, defaultValue) =>
        Promise.resolve(defaultValue),
    );
});
...
describe("QuranScreen", () => {
    ...
    it("uses web app Quran reader surface without changing ayah loading", async () => {
        ...
        await waitFor(() => {
            expect(client.getAyahsForSurahPage).toHaveBeenCalledWith(
                1,
                expect.any(Object),
            );
            expect(getByTestId("quran-web-app-reader")).toBeTruthy();
    ...
    it("loads only a small page window when opened from a far target ayah", async () => {
        ...
        client.getAyahsForSurahPage.mockImplementation(
            (surahNumber, { page, size }) => {
                const firstAyah = page * size + 1;
                const items = Array.from({ length: size }, (_, index) =>
                    mockAyah(firstAyah + index, surahNumber, {
                        pageNumber: page + 1,
                    }),
                );
        ...
        await renderQuranScreen({
            deepLinkTarget: {
                id: "quran-target-65",
                params: { surahNumber: 1, ayahNumber: 65 },
            },
        });
        ...
        const loadedPages = client.getAyahsForSurahPage.mock.calls.map(
    ...
    it("renders munasabah results from ayah detail bottom sheet", async () => {
        ...
        const { findByText, getAllByText, getByText } =
            await renderQuranScreen();
        ...
        await waitFor(() => {
            ...
            expect(
                getByText("Keterkaitan tema hidayah antar ayat."),
            ).toBeTruthy();
        });

        fireEvent.press(getByText("Al-Fatihah · Ayat 1 → Al-Baqarah · Ayat 1"));

        expect(mockNavigation.closeAndOpen).toHaveBeenCalledWith(
            "quran",
            "quran",
            {
                ayahId: 1,
        ...
        client.getHadithsForAyah.mockResolvedValue([
            {
                ...
                hadith: {
                    ...
                    book: "Shahih Bukhari",
                    number: 1,
                    translation: "Setiap amal tergantung niat.",
                },
            },
        ]);
