...
describe('asmaul wirid storage', () => {
  ...
  test('reads persisted counts and filters invalid values', async () => {
    AsyncStorage.getItem.mockResolvedValueOnce(JSON.stringify({ 1: 12, 2: 'bad', 3: 0, 4: 33.8 }));

    await expect(readAsmaulWiridCounts()).resolves.toEqual({ 1: 12, 4: 33 });
  });
  ...
  test('saves normalized counts', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('tholabul:asmaul-wirid-counts', '{"1":12}');
  });
  ...
  test('removes a count when reset to zero', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('tholabul:asmaul-wirid-counts', '{"2":5}');
  });
});
