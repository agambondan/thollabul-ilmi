    ...
    return {
        DetailHeader: ({ title, subtitle, onBack }) => (
            <View>
                ...
                {subtitle ? (
                    <Text testID='detail-subtitle'>{subtitle}</Text>
                ) : null}
            </View>
        ),
    };
...
describe("HomeScreen", () => {
    ...
    test("loads dynamic reminders into the web app daily carousel", async () => {
        ...
        expect(clientApi.getDailyReminders).toHaveBeenCalledWith({
            lang: "idn",
            limit: 20,
        });
    ...
    test("retries prayer schedule when the first call fails", async () => {
        ...
        await waitFor(() => {
            expect(
                getByText("Jadwal sholat belum tersedia. Mencoba ulang 1/4..."),
            ).toBeTruthy();
        });
    ...
    test("renders cached prayer schedule before network refresh completes", async () => {
        ...
        clientApi.getPrayerTimes.mockImplementation(
            () => new Promise(() => {}),
        );
        AsyncStorage.getItem.mockImplementation((key) => {
            if (key === "tholabul:pref:home-last-location") {
    ...
    test("retries current GPS lookup and loads prayer schedule automatically", async () => {
        ...
        await waitFor(() => {
            expect(
                getByText("GPS masih mencari lokasi. Mencoba lagi 1/5..."),
            ).toBeTruthy();
        });
    ...
    test("daily message shown when ayah fails", async () => {
        ...
        await waitFor(() => {
            expect(
                getByText("Bacaan harian belum tersedia dari server."),
            ).toBeTruthy();
        });
    });
