...
describe("layout mode normalization", () => {
    ...
    test("accepts only supported layout modes", () => {
        ...
        expect(normalizeLayoutMode(layoutModes.classic)).toBe(
            layoutModes.classic,
        );
        expect(normalizeLayoutMode(layoutModes.webApp)).toBe(
            layoutModes.webApp,
        );
    ...
    test("accepts only supported app theme preferences", () => {
        ...
        expect(normalizeThemePreference(appThemes.system)).toBe(
            appThemes.system,
        );
    ...
    test("writes normalized mode to preference storage", async () => {
        await expect(writeStoredLayoutMode("unexpected")).resolves.toBe(
            layoutModes.classic,
        );
    ...
    test("reads and writes stored app theme preference", async () => {
        ...
        AsyncStorage.getItem.mockResolvedValueOnce('"unexpected"');
        await expect(readStoredThemePreference()).resolves.toBe(
            appThemes.system,
        );

        await expect(writeStoredThemePreference("unexpected")).resolves.toBe(
            appThemes.system,
        );
        expect(AsyncStorage.setItem).toHaveBeenCalledWith(
...
describe("LayoutModeProvider", () => {
    ...
    test("falls back to classic state when stored mode is invalid", async () => {
        ...
        await waitFor(() =>
            expect(getByText(layoutModes.classic)).toBeTruthy(),
        );
        expect(getByText("ready")).toBeTruthy();
