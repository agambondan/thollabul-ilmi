...
const directLoaderTypes = new Set([
    "bookmarks",
    "feed",
    "hijri",
    "kamus",
...
describe("allFeatures", () => {
    ...
    test("remote list features declare API endpoints", () => {
        const endpointIssues = allFeatures
            ...
            .filter(
                (feature) =>
                    typeof feature.endpoint !== "string" ||
                    !feature.endpoint.startsWith("/api/"),
            )
            .map(
                (feature) => `${feature.key}:${feature.endpoint ?? "missing"}`,
            );
    ...
    test("surah content features declare supported content types", () => {
        ...
        const contentTypeIssues = allFeatures
            ...
            .filter(
                (feature) => !supportedContentTypes.has(feature.contentType),
            )
            .map(
                (feature) =>
                    `${feature.key}:${feature.contentType ?? "missing"}`,
            );
...
describe("belajarFeatureGroups", () => {
    ...
    test("matches the current Explore feature inventory", () => {
        for (const [label, expectedKeys] of Object.entries(
            expectedBelajarFeatureKeysByGroup,
        )) {
            const group = belajarFeatureGroups.find(
                (item) => item.label === label,
            );
            expect(group?.features.map((feature) => feature.key)).toEqual(
                expectedKeys,
            );
        }
    });
