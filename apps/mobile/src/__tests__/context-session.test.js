...
function TestConsumer({ onReady }) {
    ...
    return (
        <Text>{ctx.loading ? "loading" : (ctx.user?.name ?? "no-user")}</Text>
    );
}
...
describe("SessionProvider", () => {
    ...
    test("restores stored session on mount", async () => {
        session.readSession.mockResolvedValue({
            token: "tok",
            refreshToken: "rt",
            user: { name: "Ali" },
        });
...
describe("useSession", () => {
    ...
    test("signIn sets error on failure", async () => {
        ...
        await act(async () => {
            try {
                await ctx.signIn({ email: "a@b.com", password: "wrong" });
            } catch {}
        });
    ...
    test("updateCurrentUser persists merged user data", async () => {
        ...
        auth.login.mockResolvedValue({
            token: "tok",
            refreshToken: "rt",
            user: { name: "Dewi", preferred_lang: "idn" },
        });
        ...
        await act(async () => {
            await ctx.updateCurrentUser({ preferred_lang: "en" });
        });
