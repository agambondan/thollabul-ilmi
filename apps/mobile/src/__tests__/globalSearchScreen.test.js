...
describe("GlobalSearchScreen", () => {
    ...
    it("uses web app Global Search surface when web app layout is active", async () => {
        ...
        expect(
            getByPlaceholderText("Cari ayah, hadith, atau terjemahan..."),
        ).toBeTruthy();
        ...
        expect(
            getByTestId("global-search-web-app-input-wrap").props.style,
        ).toEqual(
            expect.arrayContaining([
                expect.objectContaining({ backgroundColor: "#ffffff" }),
            ]),
    ...
    it("calls searchGlobal when query changes", async () => {
        ...
        await waitFor(() => {
            expect(client.searchGlobal).toHaveBeenCalledWith(
                "fatihah",
                expect.any(Object),
            );
        });
    ...
    it("displays results for each category", async () => {
        client.searchGlobal.mockImplementation((query, { type } = {}) => {
            if (type === "ayah")
                return { ayahs: [mockAyah(1)], ayahTotal: 1, total: 1 };
            if (type === "hadith")
                return { hadiths: [mockHadith(1)], hadithTotal: 1, total: 1 };
            if (type === "doa")
                return { doas: [mockDoa(1)], doaTotal: 1, total: 1 };
            return {};
        });
    ...
    it("calls onOpenTab with correct params when pressing a result", async () => {
        ...
        const { getByTestId, getAllByTestId } = await renderGlobalSearchScreen({
            onOpenTab,
        });
