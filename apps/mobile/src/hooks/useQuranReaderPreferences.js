...
const clampTranslationFontSize = (nextSize) =>
  Math.max(
    ...
    Math.min(MAX_TRANSLATION_FONT_SIZE, Number(nextSize) || DEFAULT_TRANSLATION_FONT_SIZE),
  );

export function useQuranReaderPreferences({ onMemorizationModeChange } = {}) {
  const [fontSize, setFontSize] = useState(28);
  const [translationFontSize, setTranslationFontSize] = useState(DEFAULT_TRANSLATION_FONT_SIZE);
  ...
  const updateTranslationFontSize = useCallback(async (nextSize) => {
    ...
    await writePreference(preferenceKeys.quranTranslationFontSize, normalized);
  }, []);
  ...
  useEffect(() => {
    ...
    readPreference(preferenceKeys.quranTranslationFontSize, DEFAULT_TRANSLATION_FONT_SIZE).then((value) => {
