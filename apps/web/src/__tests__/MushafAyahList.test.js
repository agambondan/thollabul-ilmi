...
describe("MushafAyahList", () => {
    ...
    test("renders Quran font, tajweed HTML, and reader link for public route", () => {
        ...
        expect(
            screen.getByRole("link", { name: "2. Al-Baqara" }),
        ).toHaveAttribute("href", "/quran/surah/Al-Baqara#ayah-1");
        expect(container.querySelector("p.font-lpmq")).toHaveStyle({
            fontSize: "40px",
        });
        expect(
            container.querySelector("tajweed.madda_necessary"),
        ).not.toBeNull();
        expect(screen.getByText("Alif laam miim.")).toHaveStyle({
            fontSize: "16px",
        });
    });
    ...
    test("uses dashboard reader base path when supplied", () => {
        ...
        expect(
            screen.getByRole("link", { name: "2. Al-Baqara" }),
        ).toHaveAttribute("href", "/dashboard/quran/Al-Baqara#ayah-1");
    });
    ...
    test("falls back to top-level surah fields when relation payload is absent", () => {
        ...
        expect(
            screen.getByRole("link", { name: "2. Al-Baqara" }),
        ).toHaveAttribute("href", "/quran/surah/Al-Baqara#ayah-2");
    });
});
