...
const WiridClient = ({ initialItems = [] }) => {
    ...
    useEffect(() => {
        if (activeOccasion === null) return;
        if (activeOccasion === "jumat" && initialItems.length > 0) {
                ...
                {WIRID_OCCASIONS.map((w) => (
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium whitespace-nowrap transition-colors flex-shrink-0 ${
                            ...
                            (activeOccasion === null && w.occasion === "jumat")
...
export const WiridContent = ({ initialItems = [] }) => (
    <WiridClient initialItems={initialItems} />
);

export default WiridClient;
