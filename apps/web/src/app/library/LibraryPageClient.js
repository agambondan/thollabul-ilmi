...
export const LibraryContent = ({
    ...
    const [books, setBooks] = useState(() =>
        initialData ? normalizeItems(initialData) : [],
    );
    const [loading, setLoading] = useState(!initialData);
    const [loadingMore, setLoadingMore] = useState(false);
    const [hasMore, setHasMore] = useState(() =>
        initialData
            ? hasMorePages(initialData, normalizeItems(initialData).length)
            : false,
    );
        ...
        <ContentWidth compact='max-w-5xl' className='px-4'>
            ...
            {showProgressSummary && isAuthenticated && (
                <section className='mb-6 rounded-xl border border-emerald-100 bg-emerald-50/60 p-4 dark:border-slate-800 dark:bg-slate-900'>
                    <div className='mb-3 flex flex-wrap items-center justify-between gap-2'>
                        ...
                        <span className='rounded-full bg-white px-3 py-1 text-xs font-semibold text-emerald-800 dark:text-emerald-300 dark:bg-slate-800'>
                                ...
                                return (
                                    ...
                                    >
                                        <div className='mb-2 flex items-center justify-between gap-2'>
                                            <span className='rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-semibold text-emerald-800 dark:text-emerald-300 dark:bg-emerald-950/60'>
                                                {PROGRESS_LABELS[
                                                    progress.status
                                                ] ??
                                                    progress.status ??
            ...
            >
                <div className='flex items-center gap-2 rounded-xl border border-emerald-100 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-900'>
                    ...
                    <input
                        className='w-full bg-transparent text-sm text-gray-800 dark:text-gray-200 outline-none placeholder:text-gray-400'
                ...
                <select
                    className='rounded-xl border border-emerald-100 bg-white px-3 py-2 text-sm text-gray-700 dark:text-gray-200 outline-none dark:border-slate-700 dark:bg-slate-900'
            ...
            {error && !loading && (
                <div className='rounded-xl border border-red-100 bg-red-50 px-4 py-5 text-sm text-red-700 dark:text-red-400 dark:border-red-900/40 dark:bg-red-950/30'>
            ...
            {!error && !loading && filteredBooks.length === 0 && (
                <div className='rounded-xl border border-dashed border-emerald-200 bg-white px-4 py-10 text-center dark:border-slate-700 dark:bg-slate-900'>
                    <BsBook className='mx-auto mb-3 text-3xl text-emerald-700 dark:text-emerald-400' />
                            ...
                            {showProgressSummary &&
                                ...
                                progress && (
                                    <div className='mb-3 flex flex-wrap items-center gap-2'>
                                        <span className='rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-800 dark:text-emerald-300 dark:bg-emerald-950/50'>
            ...
            {hasMore && !loading && !error && (
                <div className='mt-6 flex justify-center'>
                    <button
                        className='rounded-lg border border-emerald-200 bg-white px-4 py-2 text-sm font-semibold text-emerald-800 dark:text-emerald-300 transition hover:border-emerald-300 hover:bg-emerald-50 disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-emerald-700 dark:hover:bg-slate-800'
