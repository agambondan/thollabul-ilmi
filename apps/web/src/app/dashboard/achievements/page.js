        ...
        Promise.all([
            ...
            .then(([allData, mineData, pointsData]) => {
                ...
                const earned = new Set(
                    mine.map(
                        (ua) =>
                            ua.achievement_id ?? ua.achievement?.id ?? ua.id,
                    ),
                );
                setAchievements(all);
                setEarnedIds(earned);
                setPoints(
                    pointsData?.total_points ?? pointsData?.points ?? null,
                );
            })
            ...
            {isAuthenticated && points != null && (
                <div className='bg-gradient-to-br from-amber-500 to-amber-700 rounded-2xl p-5 mb-6 text-white text-center shadow-sm'>
                    ...
                    <p className='text-xs opacity-70 mt-1'>
                        ...
                        /{achievements.length}{" "}
                        {t("achievements.badges_earned") ?? "badge diperoleh"}
                    </p>
                </div>
            )}
            ...
            {loading && (
                <div className='space-y-3'>
                    {Array.from({ length: 4 }).map((_, i) => (
                        <div
                            key={i}
                            className='p-4 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 animate-pulse'
                        >
