        ...
        totalSteps > 0
            ? Math.round(
                  (Object.keys(completed).filter(
                      (k) => k.startsWith(activeModuleId) && completed[k],
                  ).length /
                      totalSteps) *
                      100,
              )
    ...
    return (
        <div className={isWide ? "px-4 py-6" : "px-4 py-6 max-w-md mx-auto"}>
            <Link
                ...
                className='inline-flex items-center text-sm text-gray-500 hover:text-emerald-600 mb-3'
            >
                <BsChevronLeft />{" "}
                    ...
                    {modules.map((m) => (
                        ...
                        >
                            <p className='text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2'>
                                {activeModuleId === m.slug ? (
                                    <BsPlayFill className='text-emerald-500' />
                                ) : (
                ...
                <div className='sm:col-span-2'>
                    <div className='mb-4 flex items-center justify-between'>
                        <span className='text-xs font-bold text-emerald-600 uppercase tracking-wider'>
                            {activeModule.title} — {activeStepIdx + 1} /{" "}
                            {totalSteps}
                        </span>
                        <span className='text-xs text-gray-500'>
                            {moduleProgress}% selesai
                        </span>
                    </div>
