...
const StatsActivityChart = dynamic(
    () => import("./_components/StatsActivityChart"),
    { ssr: false },
);
...
const normalizeHistoryRows = (payload) => {
    const byDate = new Map(
        buildLocalPrayerRows(7).map((row) => [row.date, { ...row, count: 0 }]),
    );
...
const StatsPage = () => {
    ...
    useEffect(() => {
        ...
        const localTilawah = readLocalArray("tholabul_tilawah").map(
            normalizeTilawahEntry,
        );
        ...
        const loadApiStats = async () => {
            ...
            try {
                ...
                const tilawah = pickItems(tilawahPayload).map(
                    normalizeTilawahEntry,
                );
                ...
                setHafalCount(
                    Number(stats.hafalan ?? NaN) ||
                        hafalan.filter(isHafalanMemorized).length,
                );
                setPrayerStreak(
                    Number(stats.streak ?? NaN) || calcLocalPrayerStreak(),
                );
                ...
                setSyncError(
                    "Stat personal memakai salinan lokal karena sinkron server belum tersedia.",
                );
            }
        };
        loadApiStats();
        ...
        if (isAuthenticated) {
            achievementApi
                ...
                .then((r) => r.json())
                .then((d) =>
                    setAchievements(Array.isArray(d) ? d : (d?.data ?? [])),
                )
                .catch((e) => console.error(e));
            achievementApi
                .points()
                .then((r) => r.json())
                .then((d) =>
                    setPoints(d?.total_points ?? d?.data?.total_points ?? 0),
                )
                .catch((e) => console.error(e));
        }
    }, [isAuthenticated]);
