    ...
    return (
        <div className='mt-3 pt-3 border-t border-gray-100 dark:border-slate-700'>
            {loading ? (
                ...
                <p className='text-xs text-gray-400 mb-2'>{t('feed.no_comments') ?? 'Belum ada komentar.'}</p>
            ) : (
                        ...
                        <div key={c.id} className='flex gap-2'>
                            <div className='w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center shrink-0'>
                                <span className='text-[10px] font-bold text-blue-600 dark:text-blue-400'>
                                    {(c.author ?? c.username ?? 'U')[0].toUpperCase()}
                                </span>
                            </div>
                            <div className='flex-1 min-w-0'>
                                <p className='text-xs font-medium text-gray-700 dark:text-gray-300'>{c.author ?? c.username ?? 'User'}</p>
                                <p className='text-xs text-gray-500 dark:text-gray-400'>{c.content}</p>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {token ? (
                <div className='flex gap-2'>
                    <input
                        ...
                        placeholder={t('feed.comment_placeholder') ?? 'Tulis komentar...'}
...
export function FeedContent({ basePath = '/feed' }) {
    ...
    const handleCreate = () => {
        ...
        setCreating(true);
        feedApi.create({ caption: caption.trim(), ref_type: refType || undefined })
            .then((r) => r.json())
            .then(() => {
                setCaption('');
                setRefType('');
        ...
        <ContentWidth compact='max-w-2xl' className='px-4 py-6'>
            <div className='text-center mb-8'>
                ...
                <p className='text-sm text-gray-500 dark:text-gray-400'>
                    {t('feed.subtitle') ?? 'Bagikan dan temukan konten dari pengguna lain'}
                </p>
            </div>
            ...
            <div className='mb-6'>
                {token ? (
                    ...
                    >
                        {showCreate ? '—' : `+ ${t('feed.create') ?? 'Buat Postingan'}`}
                    </button>
                ) : (
                    <Link
                    ...
                    >
                        {t('feed.login_to_create') ?? 'Login untuk membuat postingan.'}
                    </Link>
                )}
            </div>
            ...
            {showCreate && (
                <div className='mb-6 p-4 bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700'>
                    <textarea
                        ...
                        placeholder={t('feed.content_placeholder') ?? 'Tulis caption...'}
                        rows={3}
                    ...
                    <div className='flex items-center gap-2'>
                        ...
                        >
                            <option value=''>{t('feed.ref_type') ?? 'Tipe Rujukan'}</option>
                            <option value='ayah'>{t('feed.ref_ayah') ?? 'Ayat Quran'}</option>
                            <option value='hadith'>{t('feed.ref_hadith') ?? 'Hadis'}</option>
                        </select>
            ...
            {loading ? (
                ...
                <div className='text-center py-16 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700'>
                    ...
                    <p className='text-sm text-gray-500 dark:text-gray-400'>
                        {t('feed.empty') ?? 'Belum ada postingan. Jadilah yang pertama!'}
                    </p>
                </div>
            ) : (
                <div className='space-y-4'>
                    {posts.map((post) => {
                        ...
                        const authorName = author?.name ?? author?.username ?? author?.email ?? 'User';
                        const likes = post?.likes ?? post?.like_count ?? 0;
                        const refLabel = post?.ref_type === 'ayah' ? t('feed.ref_ayah') ?? 'Ayat Quran'
                            : post?.ref_type === 'hadith' ? t('feed.ref_hadith') ?? 'Hadis' : '';

                        return (
                            <div key={post.id} className='p-5 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700'>
                                <div className='flex items-center gap-3 mb-3'>
                                    ...
                                    <div>
                                        ...
                                        <p className='text-xs text-gray-400'>{formatDate(post.created_at, lang)}</p>
                                    </div>
                                </div>
                                ...
                                {refLabel && (
                                    <div className='inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/30 rounded-lg mb-3'>
                                        <span className='text-xs font-medium text-amber-700 dark:text-amber-400'>{refLabel}</span>
                                        {post.ref_id && (
                                            <span className='text-xs text-amber-500 dark:text-amber-300'>#{post.ref_id}</span>
                                        )}
                                    </div>
                                )}
                                <div className='flex items-center gap-3 pt-3 border-t border-gray-100 dark:border-slate-700'>
                                    ...
                                    >
                                        {likes > 0 ? <BsHeartFill className='text-red-500' /> : <BsHeart />}
                                        {likes}
                                    </button>
                                    <button
                                    ...
                                    >
                                        <BsChatDots /> {t('feed.comment') ?? 'Komentar'}
                                    </button>
            {token ? (
                                        <>
                                            <button
                                ...
                                {openComment === post.id && (
                                    <CommentSection postId={post.id} lang={lang} t={t} />
                                )}
                            </div>
                        );
                    })}
