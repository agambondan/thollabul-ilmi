...
export const TafsirSurahContent = ({
    ...
    const [ayahs, setAyahs] = useState(initialSurah?.ayahs ?? []);
    const [tafsirMap, setTafsirMap] = useState(initialTafsirMap);
    const [isLoadingAyah, setIsLoadingAyah] = useState(!initialSurah);
    const [ayahLoadFailed, setAyahLoadFailed] = useState(false);
    const [isLoadingTafsir, setIsLoadingTafsir] = useState(!initialSurah);
    ...
    const hydratedSlugRef = useRef(
        initialSurah && (initialSurah.slug ?? decodedSlug) === decodedSlug
            ? decodedSlug
            : null,
    );

    const loadTafsir = (surahNumber, skipReset = false) => {
        if (!skipReset) setIsLoadingTafsir(true);
        tafsirApi
            .bySurah(surahNumber)
            .then((r) => r.json())
            .then((data) => {
    ...
    useEffect(() => {
        const skipResetForSsrHydration =
            hydratedSlugRef.current === decodedSlug;
        ...
        fetch(
            ...
            .then((data) => {
                ...
                setAyahs(data?.ayahs ?? []);
                if (data?.number) {
                    loadTafsir(data.number, skipResetForSsrHydration);
                }
            })
            .catch(() => {
                if (!skipResetForSsrHydration) setAyahLoadFailed(true);
            })
