...
export const TafsirIndexContent = ({
    ...
    const [isLoading, setIsLoading] = useState(
        !initialSurahs || initialSurahs.length === 0,
    );
    const [loadError, setLoadError] = useState(false);

    const hydratedRef = useRef(Boolean(initialSurahs?.length));

    const loadSurahs = useCallback(() => {
        const skipResetForSsrHydration = hydratedRef.current;
        ...
        fetch(`${API_URL}/api/v1/surah?size=114&sort=number`)
            ...
            .catch(() => {
                if (!skipResetForSsrHydration) setLoadError(true);
            })
                    ...
                    {filtered.map((s) => (
                        ...
                        >
                            ...
                            <div className='min-w-0'>
                                <p className='text-sm font-semibold text-gray-900 dark:text-white truncate group-hover:text-emerald-700 hover:dark:text-emerald-400 dark:group-hover:text-emerald-400 transition-colors'>
                                    {getSurahName(s, lang) ||
                                        `Surah ${s.number}`}
                                </p>
