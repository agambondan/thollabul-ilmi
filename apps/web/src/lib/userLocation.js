...
const normalizeLabel = (value, fallback = 'Lokasi GPS', prefixDistrict = true) => {
...
export const readStoredUserLocation = () => {
    ...
    try {
        const parsed = JSON.parse(localStorage.getItem(USER_LOCATION_STORAGE_KEY) || 'null');
        if (!parsed || !isFiniteNumber(parsed.lat) || !isFiniteNumber(parsed.lng)) {
            return null;
        }
        return {
...
export const writeStoredUserLocation = (location) => {
    ...
    window.dispatchEvent(new CustomEvent(USER_LOCATION_EVENT, { detail: payload }));
    return payload;
};
...
const pickAddressLabel = (address = {}) => {
    ...
    const city = address.town || address.city || address.county || address.state;
    if (city) return { value: city, prefixDistrict: false };

    return { value: '', prefixDistrict: false };
};

export const resolveLocationLabel = async (lat, lng, fallback = 'Lokasi GPS') => {
    ...
    new Promise((resolve) => {
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                const label = await resolveLocationLabel(lat, lng, fallbackLabel);
