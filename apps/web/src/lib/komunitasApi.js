...
const readLocalMessages = () => {
    ...
    try {
        return JSON.parse(localStorage.getItem(MSG_KEY) || "[]").map(
            normalizeMessage,
        );
    } catch {
        return [];
...
export const getChatMessages = async () => {
    try {
        ...
        const items = (data?.data?.items || data?.items || []).map(
            normalizeMessage,
        );
...
export const deleteChatMessage = async (id) => {
    try {
        const res = await authFetch(`/api/v1/komunitas/chat/${id}`, {
            method: "DELETE",
        });
        return res.ok;
    } catch {
...
export const subscribeChat = (onMessage) => {
    ...
    try {
        const stream = new EventSource(
            `${API_URL}/api/v1/komunitas/chat/stream`,
        );
