...
const normalizeAyah = (payload, lang, ayahBasePath) => {
    ...
    const surahSlug =
        ayah.surah?.slug ??
        ayah.surah?.translation?.latin_en?.toLowerCase()?.replace(/\s+/g, '-') ??
        '';
...
const normalizeHadith = (payload, lang, basePath) => {
    ...
    const href = slug ? (number ? `${basePath}/${slug}/${number}` : `${basePath}/${slug}`) : '';
        ...
        >
            <div className='mb-3 flex items-center justify-between gap-3'>
                <div className='flex min-w-0 items-center gap-2'>
                    ...
                    <p className={`truncate text-xs font-semibold uppercase tracking-wider ${theme.title}`}>
                        {active.title}
                    </p>
                </div>
            ...
            <div className='flex items-center justify-between gap-3 text-xs'>
                <span className={`min-w-0 truncate font-medium ${theme.source}`}>
                    {active.source}
                </span>
                {active.href ? (
                    <Link href={active.href} className='shrink-0 font-medium text-emerald-600 hover:underline dark:text-emerald-400'>
                        Selengkapnya →
                    </Link>
                ) : null}
            </div>
