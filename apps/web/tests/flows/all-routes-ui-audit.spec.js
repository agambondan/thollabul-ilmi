...
const ignoredRawKeyPatterns = [/api\/v1/i, /test@example\.com/i];

function listPageRoutes(dir = appRoot, prefix = "") {
    return fs.readdirSync(dir, { withFileTypes: true }).flatMap((entry) => {
        const entryPath = path.join(dir, entry.name);
        if (entry.isDirectory())
            return listPageRoutes(entryPath, `${prefix}/${entry.name}`);
...
function authOptionsForRoute(route) {
    if (route.startsWith("/admin")) {
        return {
            ...
            mockUser: { ...defaultMockUser, role: "admin" },
        };
    }
...
test.describe("all web routes UI audit", () => {
    ...
    for (const route of routes) {
        test(`${route} renders without route-level UI regressions`, async ({
            page,
        }) => {
            const errors = [];
            ...
            page.on("console", (msg) => {
                if (msg.type() === "warning") {
                    const text = msg.text();
                    if (
                        capturedConsoleWarningPatterns.some((pattern) =>
                            pattern.test(text),
                        )
                    ) {
                        errors.push(text);
                ...
                const text = msg.text();
                if (
                    ignoredConsoleErrorPatterns.some((pattern) =>
                        pattern.test(text),
                    )
                )
                    return;
            ...
            const globalSettingsButtonCount = await page
                .getByTestId("global-setting-button")
                .count();
            expect(
                globalSettingsButtonCount,
                `${route} global settings button count`,
            ...
            const rawKeyPattern =
                /\b(?:admin|common|dashboard|settings|mushaf|quran|hadith|link)\.[a-z0-9_.]+/i;
            const hasRawKey =
                rawKeyPattern.test(bodyText) &&
                !ignoredRawKeyPatterns.some((pattern) =>
