...
test.describe('Quran reader regression', () => {
  ...
  test('mobile reader preserves tajweed, basmalah placement, and font controls', async ({ page }) => {
    ...
    const firstAyahArabic = page.locator('#ayah-1 ul[style*="rtl"] > li').first();
    await expect(firstAyahArabic).toBeVisible();
    await expect(firstAyahArabic.locator('tajweed.madda_necessary')).toHaveCount(1);
    await expect(firstAyahArabic).not.toContainText(basmalah);
    await expect(page.locator('text=' + basmalah)).toHaveCount(1);

    const tajweedColor = await firstAyahArabic.locator('tajweed.madda_necessary').evaluate((node) =>
      getComputedStyle(node).color,
    );
    expect(tajweedColor).not.toBe('rgb(17, 24, 39)');

    await page.locator('button[aria-label="Pengaturan"]').evaluate((button) => button.click());
    ...
    const secondAyahTranslation = page
      ...
      .first();
    await page.getByRole('button', { name: 'Perbesar huruf terjemahan' }).click();
    await expect(page.getByRole('button', { name: '18px' })).toBeVisible();
    await expect(secondAyahTranslation).toHaveCSS('font-size', '18px');
  });

  test('audio range player skips next, previous, and minimizes without pausing', async ({ page }) => {
    await page.setViewportSize({ width: 412, height: 915 });
    await page.goto('/quran/surah/Al-Baqara');
    await expect(page.getByRole('heading', { name: 'Al-Baqara' })).toBeVisible();
    await dismissPermissionPrompt(page);
    ...
    const pauseCountBeforeMinimize = await page.evaluate(() => window.__audioPauseCount);
    ...
    await expect.poll(() => page.evaluate(() => window.__audioPauseCount)).toBe(pauseCountBeforeMinimize);
